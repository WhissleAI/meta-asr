# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Devanagari script (Magahi)
import os
import gc
import json
import re
...
class AudioSegment:
    ...
    transcription: str # From JSON metadata (Magahi text in Devanagari)
...
class ChunkData:
    ...
    def get_formatted_text(self) -> str:
        ...
        transcription = segment.transcription.strip() # Magahi text
...
# --- AI Annotation Functions (MODIFIED PROMPT for Magahi) ---
...
# --- MODIFIED AND MORE DETAILED PROMPT FOR MAGAHI ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for Magahi)."""
    if not genai:
    ...
    # Input texts have format: "magahi text AGE_X GENDER_Y EMOTION_Z"

    # *** REFINED MAGAHI PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator specifically for **Magahi** text written in the **Devanagari script**. Your task is to process a list of Magahi sentences, each already containing `AGE_*`, `GENDER_*`, and `EMOTION_*` metadata tags at the end. Follow these instructions with extreme precision:
...
2.  **Entity Annotation (Magahi Text Only):**
    *   Identify entities **only** within the main Magahi transcription part (before the metadata).
...
3.  **Strict Entity Tagging Format:**
    ...
    *   **Example:** For "पटना", correct annotation is `ENTITY_CITY पटना END`.
...
**Examples Demonstrating Correct Formatting (Magahi):**

*   **Input:** `"हम काल्ह पटना जाइब। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"हम काल्ह ENTITY_CITY पटना END जाइब। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"रमेश बैंक गेलऽ हई। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ENTITY_PERSON_NAME रमेश END ENTITY_ORGANIZATION बैंक END गेलऽ हई। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note: Assuming "बैंक" is Organization. Note space after each `END` unless followed by punctuation like `।`)*

*   **Input:** `"हमार बियाह दिसंबर १५ के हई। AGE_18_30 GENDER_FEMALE EMOTION_HAPPY"`
*   **Correct Output:** `"हमार बियाह ENTITY_DATE दिसंबर १५ END के हई। AGE_18_30 GENDER_FEMALE EMOTION_HAPPY INTENT_INFORM"`

*   **Input:** `"तोँ चुप रहबह का? AGE_45_60 GENDER_MALE EMOTION_ANNOYED"`
*   **Correct Output:** `"तोँ चुप रहबह का? AGE_45_60 GENDER_MALE EMOTION_ANNOYED INTENT_QUESTION"`
...
*   `... ENTITY_CITY पटना ENDजाइब ...` (Missing space after END)
*   `... हई END AGE_...` (Unnecessary END before metadata)
*   `... रमेश END बैंक END ...` (Adding END after non-entity words)
...
'''
    # *** REFINED MAGAHI PROMPT ENDS HERE ***
...
# --- Main Processing Function (Updated Comments for Magahi) ---
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Magahi audio files, extracts metadata (transcript, gender) from JSON,
    predicts age/emotion, formats text, annotates with AI (using Magahi prompt),
    ...
    if not os.path.exists(audio_dir) or not os.path.exists(metadata_dir):
        ...
        print("Please ensure your Magahi data follows the structure:")
        print(f"{base_dir}/\n  ├── audio/\n  └── metadata/")
        return

    print(f"Processing Magahi files from:\n  Audio: {audio_dir}\n  Metadata: {metadata_dir}") # Updated print
    ...
    # --- Process Files (Main loop - logic unchanged, handles Magahi data) ---
    for i, (audio_path, metadata_path) in enumerate(file_pairs):
        ...
        try:
            ...
            # 4. Create Initial Record (Uses Magahi transcript/gender from metadata)
            ...
            initial_formatted_text = chunk.get_formatted_text() # Formats with Magahi text
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                annotated_texts = annotate_batch_texts(texts_to_annotate) # Calls Magahi-specific prompt
...
if __name__ == "__main__":
    # --- *** Configuration for Magahi Data (Vaani-like Structure) *** ---
    # ---> !! IMPORTANT: UPDATE THESE PATHS TO YOUR MAGAHI DATASET !! <---
    #      It should contain the 'audio' and 'metadata' subdirectories.
    BASE_AUDIO_META_DIR = "/external4/datasets/madasr/Vaani-transcription-part/Magahi/mg" # UPDATE THIS PATH (use 'mag' or similar code)
    FINAL_OUTPUT_JSONL = "/external4/datasets/madasr/Vaani-transcription-part/Magahi/mg/mg_vaani_annotated.jsonl" # UPDATE THIS PATH
    ...
    print("Starting Magahi Audio Processing and Annotation Workflow...") # Updated print
