# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Kannada script comments/examples
import os
import gc
import json
import re
...
class AgeGenderModel(Wav2Vec2PreTrainedModel):
    def __init__(self, config):
        ...
        self.gender = ModelHead(config, 3) # Still needed for prediction if metadata fails
        self.init_weights()
...
class AudioSegment:
    ...
    end_time: float
    speaker: str
    age: float # Will come from ML model
    gender: str # Will come from JSON metadata
    transcription: str # Will come from JSON metadata
    emotion: str # Will come from ML model
    chunk_filename: str
    duration: float
...
class ChunkData:
    ...
    def get_formatted_text(self) -> str:
        ...
        # Use gender directly from segment (which comes from metadata)
        # Ensure it's uppercase for consistency with the prompt format
        gender_text = segment.gender.upper() if segment.gender else "UNKNOWN"
        emotion_text = segment.emotion.upper() # Emotion comes from model
        transcription = segment.transcription.strip() # Transcription comes from metadata
    ...
    def get_age_bucket(age: float) -> str:
        if age < 0: return "UNKNOWN" # Handle error case from ML model
...
# --- File Handling (MODIFIED for Audio/Metadata structure) ---
def get_file_pairs(audio_dir: str, metadata_dir: str) -> List[Tuple[str, str]]:
    """Finds audio files and matching JSON metadata files."""
    try:
...
# --- NEW FUNCTION: Get Info from Metadata JSON ---
def get_metadata_info(metadata_path: str) -> Tuple[Optional[str], Optional[str]]:
    """Reads transcript and gender from a JSON metadata file."""
    try:
        # *** CRITICAL: Ensure UTF-8 for Kannada script in JSON ***
        with open(metadata_path, 'r', encoding='utf-8') as f:
            data = json.load(f)

        transcript = data.get("transcript")
        gender = data.get("gender") # Expected: "Female", "Male", etc.

        if not transcript:
            print(f"Warning: 'transcript' key missing or empty in {metadata_path}")
            transcript = None # Explicitly set to None if missing/empty

        if not gender:
            print(f"Warning: 'gender' key missing or empty in {metadata_path}")
            gender = None # Explicitly set to None if missing/empty
        else:
            # Standardize gender to uppercase for consistency
             gender = gender.upper()
             # Optional: Map specific terms if needed, e.g., "Female" -> "FEMALE"
             # This simple upper() should work for "Female" and "Male".
             # Add more specific mapping if other terms are used in the JSON.
             if gender not in ["FEMALE", "MALE"]:
                  print(f"Warning: Unexpected gender value '{gender}' in {metadata_path}. Mapping to UNKNOWN.")
                  gender = "UNKNOWN" # Or handle as needed
...
# --- AI Annotation Functions (Unchanged Prompt, Modified Fixing Logic) ---

# [Functions: correct_entity_tag_spaces, fix_end_tags_and_spacing, annotate_batch_texts remain the same as in the previous version]
...
# ... (Keep the existing fix_end_tags_and_spacing function) ...
def fix_end_tags_and_spacing(text: str) -> str:
    """
    Cleans up AI-generated annotations, focusing on END tag placement,
    spacing around tags, internal spaces in tag types, and Kannada punctuation.
    Handles Kannada script characters via \w in regex.
    """
    ...
    #    Handle cases like "ENDಅನ್ನು" -> "END ಅನ್ನು"
    ...
    # 8. Kannada/Common punctuation spacing rules
    #    Remove space BEFORE specific punctuation
    text = re.sub(r'\s+([?!:;,.।])', r'\1', text) # Includes Kannada danda ।
    #    Ensure space AFTER punctuation if followed by a word character (\w handles Kannada script)
    text = re.sub(r'([?!:;,.।])(\w)', r'\1 \2', text) # Includes Kannada danda ।
...
# ... (Keep the existing annotate_batch_texts function - the prompt already uses GENDER_FEMALE/MALE) ...
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for clarity and stricter rules for Kannada)."""
    if not genai:
    ...
    # Input texts already have metadata like: "kannada text AGE_X GENDER_Y EMOTION_Z"
    # The GENDER_Y part will now be populated based on the metadata JSON.

    # *** PROMPT IS UNCHANGED AS IT EXPECTS THE CORRECT FORMAT ALREADY ***
    prompt = f'''You are an expert linguistic annotator specifically for **Kannada** text written in the **Kannada script**. Your task is to process a list of Kannada sentences, each already containing `AGE_*`, `GENDER_*`, and `EMOTION_*` metadata tags at the end. Follow these instructions with extreme precision:
...
2.  **Entity Annotation (Kannada Text Only):**
    *   Identify entities **only** within the main Kannada transcription part of the sentence (before the metadata).
...
3.  **Strict Entity Tagging Format:**
    *   **Tag Insertion:** Place the entity tag **immediately before** the identified Kannada entity text. The tag format is `ENTITY_<TYPE>` (e.g., `ENTITY_CITY`, `ENTITY_PERSON_NAME`).
    ...
    *   **`END` Tag Placement:** Place the literal string `END` **immediately after** the *complete* Kannada entity text.
    ...
    *   **Example:** For the entity "ಬೆಂಗಳೂರಿನಲ್ಲಿ", the correct annotation is `ENTITY_CITY ಬೆಂಗಳೂರಿನಲ್ಲಿ END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Kannada sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`).
...
6.  **Kannada Language Specifics:**
    *   Handle Kannada script correctly.
    *   Ensure correct spacing around Kannada punctuation (like `.`, `?`, `!`, `।`). Remove space before punctuation, ensure space after punctuation if followed by a word.
...
**Examples Demonstrating Correct Formatting (Kannada):**

*   **Input:** `"ನಾನು ನಾಳೆ ಮಾರಿಯಾಳನ್ನು ಬೆಂಗಳೂರಿನಲ್ಲಿ ಭೇಟಿಯಾಗುತ್ತೇನೆ. AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ನಾನು ನಾಳೆ ENTITY_PERSON_NAME ಮಾರಿಯಾ END ಳನ್ನು ENTITY_CITY ಬೆಂಗಳೂರಿನಲ್ಲಿ END ಭೇಟಿಯಾಗುತ್ತೇನೆ. AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"ಸಿಬಿಐನ ಈ ಎಲ್ಲಾ ವಾದಗಳನ್ನು ಮದ್ರಾಸ್ ನ್ಯಾಯಾಲಯದ ನ್ಯಾಯಾಧೀಶ ಪ್ರಕಾಶ್ ತಳ್ಳಿಹಾಕಿದ್ದಾರೆ। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ENTITY_ORGANIZATION ಸಿಬಿಐ END ನ ಈ ಎಲ್ಲಾ ವಾದಗಳನ್ನು ENTITY_LOCATION ಮದ್ರಾಸ್ END ನ್ಯಾಯಾಲಯದ ನ್ಯಾಯಾಧೀಶ ENTITY_PERSON_NAME ಪ್ರಕಾಶ್ END ತಳ್ಳಿಹಾಕಿದ್ದಾರೆ। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note: Assuming "ಮದ್ರಾಸ್" is location, "ಪ್ರಕಾಶ್" is name. Note space after each `END` unless followed by punctuation like `।`)*

*   **Input:** `"ನನ್ನ ಹುಟ್ಟುಹಬ್ಬ ಡಿಸೆಂಬರ್ 12 ರಂದು। AGE_18_30 GENDER_MALE EMOTION_HAPPY"`
*   **Correct Output:** `"ನನ್ನ ಹುಟ್ಟುಹಬ್ಬ ENTITY_DATE ಡಿಸೆಂಬರ್ 12 END ರಂದು। AGE_18_30 GENDER_MALE EMOTION_HAPPY INTENT_INFORM"`

*   **Input:** `"ನೀವು ದಯವಿಟ್ಟು ಸುಮ್ಮನಿರುತ್ತೀರಾ? AGE_45_60 GENDER_FEMALE EMOTION_ANNOYED"`
*   **Correct Output:** `"ನೀವು ದಯವಿಟ್ಟು ಸುಮ್ಮನಿರುತ್ತೀರಾ? AGE_45_60 GENDER_FEMALE EMOTION_ANNOYED INTENT_REQUEST"`

**Incorrect Examples (Common Mistakes to Avoid):**
*   `... ENTITY_PERSON_ NAM E ಮಾರಿಯಾ END ...` (Space in TYPE)
*   `... ENTITY_CITY ಬೆಂಗಳೂರಿನಲ್ಲಿEND ...` (Missing space before END)
*   `... ENTITY_CITY ಬೆಂಗಳೂರಿನಲ್ಲಿ ENDಳನ್ನು ...` (Missing space after END)
*   `... ತಳ್ಳಿಹಾಕಿದ್ದಾರೆ END AGE_...` (Unnecessary END before metadata)
*   `... ನ್ಯಾಯಾಧೀಶ END ಪ್ರಕಾಶ್ END ...` (Incorrectly adding END after non-entity words)
...
'''
    # *** REFINED KANNADA PROMPT ENDS HERE ***
...
# --- Main Processing Function (MODIFIED for Metadata) ---
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Kannada audio files, extracts metadata (transcript, gender) from JSON,
    predicts age/emotion, formats text, annotates with AI, and saves final JSONL output.
    """
    ...
    # Age/Gender Model (Still load, Age prediction needed, Gender prediction as fallback/log)
    ...
    try:
        ...
        print(f"FATAL Error loading Age/Gender model: {e}. Age prediction will fail. Exiting.")
        return
    ...
    metadata_dir = os.path.join(base_dir, "metadata") # Changed from 'text'
    ...
    if not os.path.exists(audio_dir) or not os.path.exists(metadata_dir):
        ...
        print("Please ensure your Kannada data follows the structure:")
        print(f"{base_dir}/")
        print(f"  ├── audio/    (contains .flac/.wav/.mp3 files)")
        print(f"  └── metadata/ (contains .json files with matching basenames, UTF-8 encoded)")
        return

    print(f"Processing Kannada files from:\n  Audio: {audio_dir}\n  Metadata: {metadata_dir}")
    print(f"Output will be saved to: {output_jsonl_path}")
    print("-" * 30)

    # Get matching audio-metadata pairs
    file_pairs = get_file_pairs(audio_dir, metadata_dir)
    if not file_pairs:
        print("No matching audio-metadata files found. Exiting.")
        return
    ...
    # --- Process Files ---
    for i, (audio_path, metadata_path) in enumerate(file_pairs): # Renamed text_path
        print(f"\nProcessing file {i+1}/{total_files}: {os.path.basename(audio_path)}")
        try:
            # 1. Get Transcription and Gender from Metadata JSON
            transcription, metadata_gender = get_metadata_info(metadata_path)
            if transcription is None or metadata_gender is None:
                print(f"  Skipping: Could not read required 'transcript' or 'gender' from {metadata_path}")
            ...
            try:
                ...
                if signal is None or len(signal) == 0: print(f"  Skipping: Failed to load or empty audio in {audio_path}"); continue
                duration = round(len(signal) / sr, 2)
                if duration < 0.1: print(f"  Skipping: Audio too short ({duration:.2f}s) in {audio_path}"); continue
            except Exception as load_err: print(f"  Skipping: Error loading/processing audio {audio_path}: {load_err}"); continue

            # 3. Extract Age (from ML Model) and Emotion (from ML Model)
            predicted_age, predicted_emotion = -1.0, "ERROR"
            ml_predicted_gender = "ERROR" # Store ML prediction for logging/debug if needed
            try:
                ...
                # Store ML Gender prediction result (optional, for logging)
                gender_idx_ml = torch.argmax(logits_gender_ml, dim=-1).cpu().numpy().item()
                gender_map_ml = {0: "FEMALE", 1: "MALE", 2: "OTHER"}
                ...
                predicted_emotion = extract_emotion(emotion_signal, sr, emotion_model_info)
                 ...
                 if "CUDA out of memory" in str(e): print(f"  CUDA OOM Error during Age/Emotion extraction. Skipping file."); torch.cuda.empty_cache(); gc.collect(); continue
                 else: print(f"  Runtime Error during Age/Emotion extraction: {e}"); predicted_age = -1.0; predicted_emotion = "ERROR"; ml_predicted_gender = "RUNTIME_ERROR"
            except Exception as predict_err: print(f"  Error during Age/Emotion extraction: {predict_err}"); predicted_age = -1.0; predicted_emotion = "ERROR"; ml_predicted_gender = "PREDICT_ERROR"


            # 4. Create Initial Record using Metadata and Predictions
            ...
            segment_data = AudioSegment(
                ...
                end_time=duration,
                speaker=speaker,
                age=predicted_age,            # From ML model
                gender=metadata_gender,       # <<< FROM JSON METADATA
                transcription=transcription,  # <<< FROM JSON METADATA
                emotion=predicted_emotion,    # From ML model
                chunk_filename=os.path.basename(audio_path),
                duration=duration
            )
            chunk = ChunkData(segments=[segment_data], filepath=os.path.abspath(audio_path))
            initial_formatted_text = chunk.get_formatted_text() # Uses gender/transcript from segment

            # Store raw predictions along with the formatted text for the buffer
            record = {
                "audio_filepath": chunk.filepath,
                "duration": duration,
                "initial_text": initial_formatted_text, # This now uses metadata transcript/gender
                "raw_age_output": predicted_age,
                "metadata_gender": metadata_gender,
                "ml_predicted_gender": ml_predicted_gender, # Log ML prediction
            ...
            # 5. Annotate and Save in Batches (Same logic as before)
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                annotated_texts = annotate_batch_texts(texts_to_annotate) # AI call
                        ...
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                final_record = {
                                    ...
                                    "text": annotated_text, # Final annotated text
                                    # Optional: Add raw metadata/predictions back if needed for analysis
                                    # "metadata_gender": record_data["metadata_gender"],
                                    # "ml_predicted_gender": record_data["ml_predicted_gender"],
                                    # "predicted_age_score": record_data["raw_age_output"],
                                    # "predicted_emotion": record_data["raw_emotion_prediction"],
                                }
                                ...
                                json_str = json.dumps(final_record, ensure_ascii=False) # ensure_ascii=False for Kannada
                ...
                if 'signal' in locals(): del signal; del signal_raw; del signal_mono; del emotion_signal
...
if __name__ == "__main__":
    # --- *** Configuration for Kannada Data (Vaani Structure) *** ---
    # ---> !! IMPORTANT: UPDATE THIS PATH TO YOUR KANNADA PARENT DIRECTORY !! <---
    #      It should contain the 'audio' and 'metadata' subdirectories.
    BASE_AUDIO_META_DIR = "/external4/datasets/madasr/Vaani-transcription-part/Kannada/kn" # PARENT directory
    FINAL_OUTPUT_JSONL = "/external4/datasets/madasr/Vaani-transcription-part/Kannada/kn/kn_vaani_annotated.jsonl" # Output inside the parent dir
    PROCESSING_BATCH_SIZE = 10 # Adjust based on API limits, memory, etc.
    ...
    print("Starting Kannada Audio Processing and Annotation Workflow (Vaani Structure)...")
    ...
    if not os.path.isdir(BASE_AUDIO_META_DIR):
        print(f"ERROR: Base directory not found: {BASE_AUDIO_META_DIR}")
        exit(1)
    output_dir = os.path.dirname(FINAL_OUTPUT_JSONL)
    os.makedirs(output_dir, exist_ok=True) # Ensure output dir exists

    process_audio_and_annotate(
        base_dir=BASE_AUDIO_META_DIR, # Pass the parent directory
