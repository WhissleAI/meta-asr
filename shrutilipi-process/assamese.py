# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Assamese script comments/examples
import os
import gc
import json
import re
...
# Note: Emotion model performance might vary across languages like Assamese.
...
class AudioSegment:
    ...
    age: float
    gender: str
    transcription: str # This will hold Assamese text
    emotion: str
    chunk_filename: str
    duration: float
...
def get_transcription(text_file: str) -> str:
    try:
        # *** CRITICAL: Ensure UTF-8 for Assamese script ***
...
# --- AI Annotation Functions (MODIFIED for Assamese) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    """
    Cleans up END tags and fixes spacing around tags and Assamese punctuation.
    Assumes Assamese uses similar punctuation rules as Bengali/Devanagari regarding Dari '।'.
    """
    ...
    # 8. Assamese/Common punctuation spacing rules (apply last)
    # Remove space BEFORE specific punctuation (including Assamese Dari '।')
    # This regex works for '।' as it uses the same Unicode point as Devanagari Danda
    text = re.sub(r'\s+([।?!:;,.])', r'\1', text)
    # Ensure space AFTER punctuation if followed by a word character (Assamese or Latin)
    # \w includes Unicode letters, covering Assamese script characters.
...
# *** MODIFIED PROMPT FOR ASSAMESE ***
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt adapted for Assamese)."""
    if not genai:
    ...
    # Input texts already have metadata like: "assamese text AGE_X GENDER_Y EMOTION_Z"

    # *** ASSAMESE PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator for **Assamese** text written in the **Assamese** script.
You will receive a list of Assamese sentences. Each sentence already includes metadata tags (AGE_*, GENDER_*, EMOTION_*) at the end.
...
2.  **ENTITY ANNOTATION (Assamese Text Only):** Identify entities ONLY within the Assamese transcription part of the sentence. Use ONLY the entity types from the provided list.
3.  **ENTITY TAG FORMAT (VERY IMPORTANT):**
    ...
    *   Immediately **AFTER** the Assamese entity text, add a single space followed by `END`.
    *   Example: `... ENTITY_CITY গুৱাহাটীত END ...` (using Assamese script for Guwahati)
    *   **DO NOT** add an `END` tag just before the `AGE_`, `GENDER_`, `EMOTION_` tags unless it belongs to a preceding entity.
4.  **INTENT TAG:** Determine the single primary intent of the Assamese transcription (e.g., INFORM, QUESTION, REQUEST, COMMAND, GREETING, etc.). Add ONE `INTENT_<INTENT_TYPE>` tag at the absolute end of the entire string, AFTER all other tags.
5.  **OUTPUT FORMAT:** Return a JSON array of strings, where each string is a fully annotated sentence adhering to all rules.
6.  **ASSAMESE SPECIFICS:** Handle **Assamese script**, punctuation (like the Dari `।`), and spacing correctly according to standard Assamese rules. Ensure proper spacing around the inserted tags.
...
**Example Input String (Assamese):**
"মই কাইলৈ মাৰিয়াক গুৱাহাটীত দেখিছিলোঁ। AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL"

**CORRECT Example Output String (Assamese):**
"মই কাইলৈ ENTITY_PERSON_NAME মাৰিয়া END ক ENTITY_CITY গুৱাহাটীত END দেখিছিলোঁ। AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"

**INCORRECT Example Output String (Spaces in Tag):**
"মই কাইলৈ ENTITY_PERSON_ NAM E মাৰিয়া END ক ENTITY_CIT Y গুৱাহাটীত END দেখিছিলোঁ। AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"

**INCORRECT Example Output String (Extra END before metadata):**
"মই কাইলৈ ENTITY_PERSON_NAME মাৰিয়া END ক ENTITY_CITY গুৱাহাটীত END দেখিছিলোঁ। END AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"


**Sentences to Annotate Now:**
{json.dumps(texts_to_annotate, ensure_ascii=False, indent=2)}
'''
    # *** ASSAMESE PROMPT ENDS HERE ***
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     if isinstance(sentence, str):
                          ...
                          # 2. Fix END tags and spacing (using Assamese-aware function)
...
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Assamese audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Assamese prompt), and saves final JSONL output.
    """
    ...
    try:
        ...
        # *** CRITICAL: Ensure UTF-8 for writing Assamese output ***
    ...
    # Emotion Model (Largely Language Agnostic, but monitor performance on Assamese)
    ...
    # --- Prepare File Paths (Using base_dir for Assamese data) ---
    ...
    for i, (audio_path, text_path) in enumerate(file_pairs):
        ...
        try:
            # 1. Get Assamese Transcription (Ensures UTF-8)
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                # Call Gemini for annotation (with retries and Assamese prompt)
                ...
                if len(annotated_texts) != current_batch_size:
                    ...
                    try:
                        ...
                        # *** CRITICAL: Ensure UTF-8 for appending Assamese output ***
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                ...
                                # *** CRITICAL: ensure_ascii=False for Assamese ***
...
if __name__ == "__main__":
    # --- *** Configuration for Assamese Data *** ---
    # 1. SET THE BASE DIRECTORY CONTAINING YOUR ASSAMESE 'audio' and 'text' subfolders
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/assamese/as"  # <--- CHANGE THIS TO YOUR ASSAMESE DATA PATH

    # 2. SET THE DESIRED OUTPUT FILENAME FOR THE ANNOTATED ASSAMESE DATA
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/assamese/as/as_annotated_data.jsonl" # <--- CHANGE THIS (e.g., as for Assamese)
    ...
    print("Starting Assamese Audio Processing and Annotation Workflow...") # Updated print
