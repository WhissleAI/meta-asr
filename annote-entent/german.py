...
def remove_existing_tags(text):
    ...
    Handles German accented characters properly.
    """
    preserved_tags = {
...
def fix_end_tags(text):
    ...
    Handles German text with proper spacing rules.
    """
    if not isinstance(text, str):
        return text

    # Handle German-specific punctuation spacing
...
def annotate_sentences(sentences):
    prompt = f'''
You are given a list of German sentences that may contain existing tags like GENDER_FEMALE, EMOTION_NEU, AGE_45_60, and SPEAKER_CHANGE.

Your task is to:
1. Remove any legacy entity tags such as NER_PERSON, NER_NORP, etc.
2. Preserve these existing tags exactly as they appear.
3. **ONLY annotate entities from the following list in German text.** Do NOT invent new entity types.
4. Insert entity tags in the format ENTITY_<TYPE> before each entity and append " END" after it.
5. Focus on identifying German entities that match the provided list.
6. Classify the **intent** of each German sentence and add an **INTENT_<INTENT_TYPE>** tag at the end.
7. Return the output as a JSON array of annotated sentences.

IMPORTANT:
- Maintain proper German spacing rules (space before :;?! and no space before .,)
- Handle German accented characters and special characters properly
- Follow German capitalization rules for proper nouns and entities
...
Example in German:
Input: "Ich habe Marie gestern in Paris gesehen. AGE_30_40 GENDER_FEMALE EMOTION_JOY SPEAKER_CHANGE"
Output: "Ich habe ENTITY_PERSON_NAME Marie END gestern in ENTITY_CITY Paris END gesehen. AGE_30_40 GENDER_FEMALE EMOTION_JOY SPEAKER_CHANGE INTENT_INFORM"
...
def process_jsonl_file(input_path, output_path, batch_size=10):
    """
    Process German JSONL file with proper character encoding and German text handling.
    """
...
if __name__ == "__main__":
    input_jsonl_path = "/hydra2-prev/home/compute/workspace_himanshu/Processed_Data/ner_metadata/german_part2.jsonl"
    output_jsonl_path = "/hydra2-prev/home/compute/workspace_himanshu/Processed_Data/gemini_metadata/german_exta.jsonl"
