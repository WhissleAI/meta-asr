# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Malayalam script comments/examples
import os
import gc
import json
import re
...
# Note: Emotion model performance might vary across languages like Malayalam.
...
class AudioSegment:
    ...
    age: float
    gender: str
    transcription: str # This will hold Malayalam text
    emotion: str
    chunk_filename: str
    duration: float
...
def get_transcription(text_file: str) -> str:
    """Reads Malayalam transcription from a text file."""
    try:
        # *** CRITICAL: Ensure UTF-8 for Malayalam script ***
...
# --- AI Annotation Functions (MODIFIED for Malayalam) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    ...
    Handles Malayalam script characters via \w in regex.
    """
    ...
    #    Ensure space AFTER punctuation if followed by a word character (\w handles Malayalam script)
...
# --- MODIFIED AND MORE DETAILED PROMPT FOR MALAYALAM ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for clarity and stricter rules for Malayalam)."""
    if not genai:
    ...
    # Input texts already have metadata like: "malayalam text AGE_X GENDER_Y EMOTION_Z"

    # *** REFINED MALAYALAM PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator specifically for **Malayalam** text written in the **Malayalam** script. Your task is to process a list of Malayalam sentences, each already containing `AGE_*`, `GENDER_*`, and `EMOTION_*` metadata tags at the end. Follow these instructions with extreme precision:
...
2.  **Entity Annotation (Malayalam Text Only):**
    *   Identify entities **only** within the main Malayalam transcription part of the sentence (before the metadata).
...
3.  **Strict Entity Tagging Format:**
    *   **Tag Insertion:** Place the entity tag **immediately before** the identified Malayalam entity text. The tag format is `ENTITY_<TYPE>` (e.g., `ENTITY_CITY`, `ENTITY_PERSON_NAME`).
    ...
    *   **`END` Tag Placement:** Place the literal string `END` **immediately after** the *complete* Malayalam entity text.
    ...
    *   **Example:** For the entity "കൊച്ചി", the correct annotation is `ENTITY_CITY കൊച്ചി END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Malayalam sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`).
...
6.  **Malayalam Language Specifics:**
    *   Handle Malayalam script correctly.
...
*   **Input:** `"നാളെ ഞാൻ മരിയയെ കൊച്ചിയിൽ കാണും. AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"നാളെ ഞാൻ ENTITY_PERSON_NAME മരിയ END യെ ENTITY_CITY കൊച്ചി END യിൽ കാണും. AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"സിബിഐയുടെ ഈ വാദങ്ങളെല്ലാം മദ്രാസ് കോടതി ജഡ്ജി പ്രകാശ് തള്ളി. AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ENTITY_ORGANIZATION സിബിഐ END യുടെ ഈ വാദങ്ങളെല്ലാം ENTITY_LOCATION മദ്രാസ് END കോടതി ജഡ്ജി ENTITY_PERSON_NAME പ്രകാശ് END തള്ളി. AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note: Assuming "മദ്രാസ്" is the location and "പ്രകാശ്" is the person's name. Also note the space after each `END`)*

*   **Input:** `"എൻ്റെ ജന്മദിനം ഡിസംബർ 12 നാണ്. AGE_18_30 GENDER_MALE EMOTION_HAPPY"`
*   **Correct Output:** `"എൻ്റെ ജന്മദിനം ENTITY_DATE ഡിസംബർ 12 END നാണ്. AGE_18_30 GENDER_MALE EMOTION_HAPPY INTENT_INFORM"`

*   **Input:** `"നിങ്ങൾ ദയവായി മിണ്ടാതിരിക്കുമോ? AGE_45_60 GENDER_FEMALE EMOTION_ANNOYED"`
*   **Correct Output:** `"നിങ്ങൾ ദയവായി മിണ്ടാതിരിക്കുമോ? AGE_45_60 GENDER_FEMALE EMOTION_ANNOYED INTENT_REQUEST"`

**Incorrect Examples (Common Mistakes to Avoid):**
*   `... ENTITY_PERSON_ NAM E മരിയ END ...` (Space in TYPE)
*   `... ENTITY_CITY കൊച്ചിEND ...` (Missing space before END)
*   `... ENTITY_CITY കൊച്ചി ENDയിൽ ...` (Missing space after END)
*   `... തള്ളി END AGE_...` (Unnecessary END before metadata)
*   `... ജഡ്ജി END പ്രകാശ് END ...` (Incorrectly adding END after non-entity words)
...
'''
    # *** REFINED MALAYALAM PROMPT ENDS HERE ***
...
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Malayalam audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Malayalam prompt), and saves final JSONL output.
    """
    ...
    for i, (audio_path, text_path) in enumerate(file_pairs):
        ...
        try:
            # 1. Get Transcription (Ensures UTF-8 for Malayalam)
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                # Use the Malayalam-specific annotation function
                ...
                if len(annotated_texts) != current_batch_size:
                    ...
                    try:
                        ...
                        # Ensure UTF-8 for writing Malayalam text
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                final_record = {
                                    ...
                                    "text": annotated_text, # Contains final Malayalam text with tags
                                }
...
if __name__ == "__main__":
    # --- *** Configuration for Malayalam Data *** ---
    # >>> IMPORTANT: CHANGE THESE PATHS TO YOUR MALAYALAM DATA LOCATION <<<
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/malayalam/ml"  # Example: /data/malayalam_corpus/ml
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/malayalam/ml/ml_annotated_output.jsonl" # Example: /data/malayalam_corpus/ml/annotated_malayalam.jsonl
    ...
    print("Starting Malayalam Audio Processing and Annotation Workflow...")
