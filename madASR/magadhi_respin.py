# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Magadhi script (Devanagari) comments/examples
import os
import gc
import json
import re
...
def fix_end_tags_and_spacing(text: str) -> str:
    """
    Cleans up AI-generated annotations for Magadhi text (Devanagari), focusing on END tag placement,
...
# --- *** MODIFIED PROMPT FOR MAGADHI *** ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt modified for Magadhi)."""
    if not genai:
    ...
    # Input texts have metadata like: "magadhi text AGE_X GENDER_Y EMOTION_Z DOMAIN_W" (DOMAIN_W is already capitalized)

    # *** MODIFIED MAGADHI PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator specifically for **Magadhi** text written in the **Devanagari script**. Your task is to process a list of Magadhi sentences, each already containing `AGE_*`, `GENDER_*`, `EMOTION_*`, and `DOMAIN_*` metadata tags at the end. Follow these instructions with extreme precision:
...
2.  **Entity Annotation (Magadhi Text Only):**
    *   Identify entities **only** within the main Magadhi transcription part of the sentence (before the metadata).
...
3.  **Strict Entity Tagging Format:**
    *   **Tag Insertion:** Place the entity tag **immediately before** the identified Magadhi entity text. The tag format is `ENTITY_<TYPE>` (e.g., `ENTITY_CITY`, `ENTITY_PERSON_NAME`).
    ...
    *   **`END` Tag Placement:** Place the literal string `END` **immediately after** the *complete* Magadhi entity text.
    ...
    *   **Example:** For the Magadhi entity "पटना" (Patna), the correct annotation is `ENTITY_CITY पटना END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Magadhi sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`, `OTHER`).
...
**Examples Demonstrating Correct Formatting (Magadhi - Using Devanagari script):**
*(Note: These Magadhi examples are illustrative of the structure)*

*   **Input:** `"हमर नाम रमेश हई। AGE_30-45 GENDER_MALE EMOTION_NEUTRAL DOMAIN_GENERAL"`
*   **Correct Output:** `"हमर नाम ENTITY_PERSON_NAME रमेश END हई। AGE_30-45 GENDER_MALE EMOTION_NEUTRAL DOMAIN_GENERAL INTENT_INFORM"`

*   **Input:** `"ऊ पटना में रहई छे। AGE_45-60 GENDER_MALE EMOTION_NEUTRAL DOMAIN_LOCATION"`
*   **Correct Output:** `"ऊ ENTITY_CITY पटना END में रहई छे। AGE_45-60 GENDER_MALE EMOTION_NEUTRAL DOMAIN_LOCATION INTENT_INFORM"`

*   **Input:** `"मीटिंग १० मई के होतई। AGE_18-24 GENDER_MALE EMOTION_NEUTRAL DOMAIN_MEETING"`
*   **Correct Output:** `"मीटिंग ENTITY_DATE १० मई END के होतई। AGE_18-24 GENDER_MALE EMOTION_NEUTRAL DOMAIN_MEETING INTENT_INFORM"`

*   **Input:** `"कृपया चुप रहऽ? AGE_60+ GENDER_MALE EMOTION_ANGRY DOMAIN_INSTRUCTION"`
*   **Correct Output:** `"कृपया चुप रहऽ? AGE_60+ GENDER_MALE EMOTION_ANGRY DOMAIN_INSTRUCTION INTENT_REQUEST"`
...
*   `... रहई छे END AGE_...` (Unnecessary END before metadata)
...
'''
    # *** MODIFIED MAGADHI PROMPT ENDS HERE ***
        ...
        try:
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     # Apply Magadhi fixing rules (uses Devanagari rules)
...
# --- *** Main Processing Function for Magadhi *** ---
def process_magadhi_json(input_json_path: str, output_jsonl_path: str, base_audio_path: Optional[str], batch_size: int = 10) -> None:
    """
    Processes a Magadhi JSON file (assuming similar structure to original script input),
    determines age group via fallback logic (reassigned -> original -> predicted), predicts emotion,
    constructs audio paths (absolute if base provided), annotates text with Gemini (using Magadhi prompt),
    capitalizes domain tag, and saves final JSONL output.

    Args:
        input_json_path: Path to the input JSON file (expected to contain Magadhi data).
    ...
    try:
        print(f"Loading Magadhi data from {input_json_path}...")
    ...
    print(f"Starting processing of {total_records} Magadhi records...")
    ...
    for i, (record_key, record_data) in enumerate(records_to_process):
        ...
        # *** IMPORTANT: Verify these keys match your Magadhi JSON structure ***
        ...
        if missing_keys:
            print(f"  Skipping: Record missing required keys: {', '.join(missing_keys)} (Verify keys in Magadhi JSON!)"); skipped_records_count += 1; continue

        wav_path_relative = record_data["wav_path"]
        text = record_data["text"] # Should be Magadhi text in Devanagari
        ...
        # Uses 'age_group_reassigned', 'age_group' keys - verify these exist in your Magadhi JSON
        ...
        # Ensure text is clean Magadhi script (Devanagari) before adding tags
        ...
        if len(processed_records_buffer) >= batch_size or (i + 1) == total_records:
            ...
            print(f"\n--- Annotating Batch {batch_num} ({current_batch_size} records) with Magadhi Prompt ---")
            texts_to_annotate = [rec["text_for_gemini"] for rec in processed_records_buffer]
            annotated_texts = annotate_batch_texts(texts_to_annotate) # Uses Magadhi prompt
                    ...
                    with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                        for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                            final_record = {
                                ...
                                "text": annotated_text, # Contains Magadhi text + tags
                                "duration": record_data["duration"]
                            }
    ...
    print("Note: Ensure input JSON keys ('wav_path', 'text', 'gender_reassigned', etc.) match the script for your Magadhi data.")
    print("="*30)


# --- Main Execution ---
if __name__ == "__main__":
    # --- *** Configuration for Magadhi *** ---
    # !!! IMPORTANT: Update these paths for your Magadhi dataset !!!
    INPUT_JSON_FILE = "/external4/datasets/MADASR/IISc_RESPIN_train_small/data/mg_meta_data.json" # e.g., /data/magadhi/metadata.json
    FINAL_OUTPUT_JSONL = "/external4/datasets/MADASR/process_train_small/magadhi_output.jsonl" # e.g., /data/magadhi_processed/output.jsonl
    ...
    BASE_AUDIO_DIRECTORY = "/external4/datasets/MADASR/IISc_RESPIN_train_small" # e.g., "/data/magadhi/wavs" or None
    PROCESSING_BATCH_SIZE = 10 # Adjust based on API limits and memory

    # --- Verifications ---
    print("--- Magadhi Data Processing Script ---")
    load_dotenv()
    ...
    print("\nStarting Magadhi JSON Processing with Age Fallback Logic...")
    ...
    # Call the Magadhi processing function
    process_magadhi_json(
    ...
    print("\nMagadhi Workflow with Age Fallback complete.")
