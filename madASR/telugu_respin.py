...
def fix_end_tags_and_spacing(text: str) -> str:
    ...
    Assumes standard punctuation like '.', '?', '!' are used in digital Telugu text.
    """
...
# --- *** MODIFIED PROMPT FOR TELUGU *** ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt modified for Telugu)."""
    if not genai:
    ...
    # Input texts have metadata like: "telugu text AGE_X GENDER_Y EMOTION_Z DOMAIN_W" (DOMAIN_W is already capitalized)

    # *** MODIFIED TELUGU PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator specifically for **Telugu** text written in the **Telugu script**. Your task is to process a list of Telugu sentences, each already containing `AGE_*`, `GENDER_*`, `EMOTION_*`, and `DOMAIN_*` metadata tags at the end. Follow these instructions with extreme precision:
...
3.  **Strict Entity Tagging Format:**
    ...
    *   **Example:** For the Telugu entity "హైదరాబాద్", the correct annotation is `ENTITY_CITY హైదరాబాద్ END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Telugu sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`, `OTHER`).
...
**Examples Demonstrating Correct Formatting (Telugu - Using Telugu script):**

*   **Input:** `"నా పేరు రమేష్. AGE_30-45 GENDER_MALE EMOTION_NEUTRAL DOMAIN_GENERAL"`
*   **Correct Output:** `"నా పేరు ENTITY_PERSON_NAME రమేష్ END. AGE_30-45 GENDER_MALE EMOTION_NEUTRAL DOMAIN_GENERAL INTENT_INFORM"`

*   **Input:** `"అతను హైదరాబాద్ లో ఉంటాడు. AGE_45-60 GENDER_MALE EMOTION_NEUTRAL DOMAIN_LOCATION"`
*   **Correct Output:** `"అతను ENTITY_CITY హైదరాబాద్ END లో ఉంటాడు. AGE_45-60 GENDER_MALE EMOTION_NEUTRAL DOMAIN_LOCATION INTENT_INFORM"`

*   **Input:** `"సమావేశం మే 10న జరుగుతుంది. AGE_18-24 GENDER_MALE EMOTION_NEUTRAL DOMAIN_MEETING"`
*   **Correct Output:** `"సమావేశం ENTITY_DATE మే 10న END జరుగుతుంది. AGE_18-24 GENDER_MALE EMOTION_NEUTRAL DOMAIN_MEETING INTENT_INFORM"`

*   **Input:** `"దయచేసి నిశ్శబ్దంగా ఉండండి? AGE_60+ GENDER_MALE EMOTION_ANGRY DOMAIN_INSTRUCTION"`
*   **Correct Output:** `"దయచేసి నిశ్శబ్దంగా ఉండండి? AGE_60+ GENDER_MALE EMOTION_ANGRY DOMAIN_INSTRUCTION INTENT_REQUEST"`
...
*   `... ఉంటాడు END AGE_...` (Unnecessary END before metadata)
*   `... మే END 10న END ...` (Incorrectly adding END after parts of a multi-word entity)
...
'''
    # *** MODIFIED TELUGU PROMPT ENDS HERE ***
        ...
        try:
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     # Apply Telugu fixing rules (uses general tag/punctuation rules)
...
# --- *** Main Processing Function for Telugu *** ---
def process_telugu_json(input_json_path: str, output_jsonl_path: str, base_audio_path: Optional[str], batch_size: int = 10) -> None:
    """
    Processes a Telugu JSON file (assuming similar structure to original script input),
    determines age group via fallback logic (reassigned -> original -> predicted), predicts emotion,
    constructs audio paths (absolute if base provided), annotates text with Gemini (using Telugu prompt),
    capitalizes domain tag, and saves final JSONL output.

    Args:
        input_json_path: Path to the input JSON file (expected to contain Telugu data).
    ...
    try:
        print(f"Loading Telugu data from {input_json_path}...")
    ...
    print(f"Starting processing of {total_records} Telugu records...")
    ...
    for i, (record_key, record_data) in enumerate(records_to_process):
        ...
        # *** IMPORTANT: Verify these keys match your Telugu JSON structure ***
        ...
        if missing_keys:
            print(f"  Skipping: Record missing required keys: {', '.join(missing_keys)} (Verify keys in Telugu JSON!)"); skipped_records_count += 1; continue

        wav_path_relative = record_data["wav_path"]
        text = record_data["text"] # Should be Telugu text
        ...
        # Uses 'age_group_reassigned', 'age_group' keys - verify these exist in your Telugu JSON
        ...
        # Ensure text is clean Telugu script before adding tags
        ...
        if len(processed_records_buffer) >= batch_size or (i + 1) == total_records:
            ...
            print(f"\n--- Annotating Batch {batch_num} ({current_batch_size} records) with Telugu Prompt ---")
            texts_to_annotate = [rec["text_for_gemini"] for rec in processed_records_buffer]
            annotated_texts = annotate_batch_texts(texts_to_annotate) # Uses Telugu prompt
                    ...
                    with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                        for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                            final_record = {
                                ...
                                "text": annotated_text, # Contains Telugu text + tags
                                "duration": record_data["duration"]
                            }
                            ...
                            json_str = json.dumps(final_record, ensure_ascii=False) # ensure_ascii=False for Telugu script
    ...
    print("Note: Ensure input JSON keys ('wav_path', 'text', 'gender_reassigned', etc.) match the script for your Telugu data.")
    print("="*30)


# --- Main Execution ---
if __name__ == "__main__":
    # --- *** Configuration for Telugu *** ---
    # !!! IMPORTANT: Update these paths for your Telugu dataset !!!
    INPUT_JSON_FILE = "/external4/datasets/MADASR/IISc_RESPIN_train_small/data/te_meta_data.json" # e.g., /data/telugu/metadata.json
    FINAL_OUTPUT_JSONL = "/external4/datasets/MADASR/process_train_small/telugu_output.jsonl" # e.g., /data/telugu_processed/output.jsonl
    ...
    BASE_AUDIO_DIRECTORY = "/external4/datasets/MADASR/IISc_RESPIN_train_small" # e.g., "/data/telugu/wavs" or None
    PROCESSING_BATCH_SIZE = 10 # Adjust based on API limits and memory

    # --- Verifications ---
    print("--- Telugu Data Processing Script ---")
    load_dotenv()
    ...
    print("\nStarting Telugu JSON Processing with Age Fallback Logic...")
    ...
    # Call the Telugu processing function
    process_telugu_json(
    ...
    print("\nTelugu Workflow with Age Fallback complete.")
