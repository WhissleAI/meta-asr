# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Nepali script comments/examples
import os
import gc
import json
import re
...
# Note: Emotion model performance might vary across languages like Nepali.
...
class AudioSegment:
    ...
    age: float
    gender: str
    transcription: str # This will hold Nepali text
    emotion: str
    chunk_filename: str
    duration: float
...
def get_transcription(text_file: str) -> str:
    """Reads Nepali transcription from a text file."""
    try:
        # *** CRITICAL: Ensure UTF-8 for Nepali script (Devanagari) ***
...
# --- AI Annotation Functions (MODIFIED for Nepali) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    ...
    Handles Nepali script characters (Devanagari) via \w in regex.
    """
...
# --- MODIFIED AND MORE DETAILED PROMPT FOR NEPALI (Devanagari Script Focus) ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for clarity and stricter rules for Nepali in Devanagari script)."""
    if not genai:
    ...
    # Input texts already have metadata like: "nepali text AGE_X GENDER_Y EMOTION_Z"

    # *** REFINED NEPALI PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator specifically for **Nepali** text written in the **Devanagari** script. Your task is to process a list of Nepali sentences, each already containing `AGE_*`, `GENDER_*`, and `EMOTION_*` metadata tags at the end. Follow these instructions with extreme precision:
...
2.  **Entity Annotation (Nepali Text Only):**
    *   Identify entities **only** within the main Nepali transcription part of the sentence (before the metadata).
...
3.  **Strict Entity Tagging Format:**
    *   **Tag Insertion:** Place the entity tag **immediately before** the identified Nepali entity text. The tag format is `ENTITY_<TYPE>` (e.g., `ENTITY_CITY`, `ENTITY_PERSON_NAME`).
    ...
    *   **`END` Tag Placement:** Place the literal string `END` **immediately after** the *complete* Nepali entity text.
    ...
    *   **Example:** For the entity "काठमाडौँ", the correct annotation is `ENTITY_CITY काठमाडौँ END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Nepali sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`).
...
6.  **Nepali Language Specifics:**
    *   Handle Nepali script (Devanagari) correctly.
...
**Allowed ENTITY TYPES (Use Only These):**
    [ "PERSON_NAME", "ORGANIZATION", "LOCATION", "ADDRESS", "CITY", "STATE", "DISTRICT", "COUNTRY", "ZIP_CODE", "CURRENCY", "PRICE", "DATE", # Added DISTRICT
    "TIME", "DURATION", "APPOINTMENT_DATE", "APPOINTMENT_TIME", "DEADLINE", "DELIVERY_DATE", "DELIVERY_TIME", "EVENT", "MEETING", "FESTIVAL", # Added FESTIVAL
    "TASK", "PROJECT_NAME", "ACTION_ITEM", "PRIORITY", "FEEDBACK", "REVIEW", "RATING", "COMPLAINT", "QUESTION", "RESPONSE", "NOTIFICATION_TYPE", "AGENDA",
    "REMINDER", "NOTE", "RECORD", "ANNOUNCEMENT", "UPDATE", "SCHEDULE", "BOOKING_REFERENCE", "APPOINTMENT_NUMBER", "ORDER_NUMBER", "INVOICE_NUMBER", "PAYMENT_METHOD", "PAYMENT_AMOUNT", "BANK_NAME", "ACCOUNT_NUMBER", "CREDIT_CARD_NUMBER", "TAX_ID", "CITIZENSHIP_NUMBER", "DRIVER'S_LICENSE", "PASSPORT_NUMBER", "INSURANCE_PROVIDER", "POLICY_NUMBER", "INSURANCE_PLAN", "CLAIM_NUMBER", "POLICY_HOLDER", "BENEFICIARY", # Changed SOCIAL_SECURITY_NUMBER to CITIZENSHIP_NUMBER
    ...
    # Added DISTRICT, FESTIVAL. Changed SOCIAL_SECURITY_NUMBER to CITIZENSHIP_NUMBER.

**Examples Demonstrating Correct Formatting:**

*   **Input:** `"म भोलि रामलाई पोखरामा भेट्छु। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"म भोलि ENTITY_PERSON_NAME राम END लाई ENTITY_CITY पोखरा END मा भेट्छु। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"तपाईंको नाम के हो? AGE_18_30 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"तपाईंको नाम के हो? AGE_18_30 GENDER_FEMALE EMOTION_NEUTRAL INTENT_QUESTION"`

*   **Input:** `"कृपया यहाँ बस्नुहोस्। AGE_45_60 GENDER_MALE EMOTION_POLITE"`
*   **Correct Output:** `"कृपया यहाँ बस्नुहोस्। AGE_45_60 GENDER_MALE EMOTION_POLITE INTENT_REQUEST"`

*   **Input:** `"दशैंमा हामी घर जान्छौं। AGE_18_30 GENDER_MALE EMOTION_HAPPY"`
*   **Correct Output:** `"ENTITY_FESTIVAL दशैं END मा हामी घर जान्छौं। AGE_18_30 GENDER_MALE EMOTION_HAPPY INTENT_INFORM"`


**Incorrect Examples (Common Mistakes to Avoid):**
*   `... ENTITY_PERSON_ NAM E राम END ...` (Space in TYPE)
*   `... ENTITY_CITY पोखराEND ...` (Missing space before END)
*   `... ENTITY_CITY पोखरा ENDमा ...` (Missing space after END)
*   `... भेट्छु। END AGE_...` (Unnecessary END before metadata)
*   `... राम END लाई END ...` (Incorrectly adding END after non-entity words)
...
'''
    # *** REFINED NEPALI PROMPT ENDS HERE ***
    ...
    for attempt in range(max_retries):
        try:
            ...
            if json_match:
                ...
                # Check again if it now looks like a JSON list
                if assistant_reply.startswith('[') and assistant_reply.endswith(']'):
                    ...
                    # Fallback: Attempt to parse the whole reply if it might be just the JSON list without markdown
                    if assistant_reply.startswith('[') and assistant_reply.endswith(']'):
                        json_str = assistant_reply
                    else:
            ...
            try:
                ...
                print("Extracted string snippet:", json_str[:500]) # Print more for debugging
                # Attempt a more lenient parse (e.g., handling trailing commas, though less likely needed with search)
                try:
                    # This requires a library like `demjson` or custom logic. Sticking to standard json for now.
                    print("Standard JSON parsing failed.")
                    raise json_e # Re-raise original error
                except Exception as lenient_e:
                    print(f"Lenient JSON parsing also failed: {lenient_e}")
                    raise json_e # Re-raise original error
            ...
            # Handle potential content safety blocks
            elif "response was blocked" in str(e).lower() or "safety setting" in str(e).lower():
                 print(f"Warning: Response potentially blocked due to safety settings (Attempt {attempt+1}).")
                 # Option: Return with a specific error tag for this batch item? Or retry?
                 # For now, let's assume it's a temporary issue and retry might help, or fail eventually.
                 time.sleep(retry_delay * (attempt + 1))
            else:
                 time.sleep(retry_delay * (attempt + 1))

            if attempt == max_retries - 1:
                 # Add specific error tag based on the exception if possible
                 error_tag = "ANNOTATION_ERROR_API_CALL"
                 if "safety setting" in str(e).lower():
                     error_tag = "ANNOTATION_ERROR_SAFETY_BLOCK"
                 elif "rate limit" in str(e).lower():
                     error_tag = "ANNOTATION_ERROR_RATE_LIMIT"

                 return [fix_end_tags_and_spacing(t) + f" {error_tag}" for t in texts_to_annotate]
...
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Nepali audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Nepali prompt), and saves final JSONL output.
    """
    ...
    for i, (audio_path, text_path) in enumerate(file_pairs):
        ...
        try:
            # 1. Get Transcription (Ensures UTF-8 for Nepali)
            ...
            try:
                ...
                if sr_orig != target_sr:
                    ...
                    # Handle potential multi-channel audio before resampling
                    if signal_float.ndim > 1:
                         signal_mono = np.mean(signal_float, axis=1)
                    else:
                         signal_mono = signal_float
                    signal = librosa.resample(y=signal_mono, orig_sr=sr_orig, target_sr=target_sr)
                    sr = target_sr
                else:
                    signal_float = signal_raw.astype(np.float32) if not np.issubdtype(signal_raw.dtype, np.floating) else signal_raw
                    # Handle potential multi-channel audio even if sr matches
                    if signal_float.ndim > 1:
                        signal = np.mean(signal_float, axis=1)
                    else:
                        signal = signal_float
                    sr = sr_orig

                # Final check for mono (should be redundant now but safe)
            ...
            try:
                # Limit input length for stability/memory
                max_len_inference_samples = 30 * sr
                inference_signal = signal[:max_len_inference_samples]

                inputs = processor(inference_signal, sampling_rate=sr, return_tensors="pt", padding=True)
                ...
                gender = gender_map.get(gender_idx, "UNKNOWN")
                 ...
                 if "CUDA out of memory" in str(e):
                     ...
                     age = -1.0; gender = "ERROR"
            except Exception as age_gender_err:
                print(f"  Error during Age/Gender extraction: {age_gender_err}")
                age = -1.0; gender = "ERROR"

            # 4. Extract Emotion (Use the same potentially truncated signal)
            emotion = "ERROR"
            try:
                 # Use the same signal length as age/gender for consistency if already defined
                 emotion_signal = inference_signal if 'inference_signal' in locals() else signal[:max_len_inference_samples]
                 emotion = extract_emotion(emotion_signal, sr, emotion_model_info)
            except Exception as emotion_err:
                 print(f"  Error during Emotion extraction: {emotion_err}")
                 emotion = "ERROR"
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                # Use the Nepali-specific annotation function
                ...
                if len(annotated_texts) != current_batch_size:
                    ...
                    try:
                        ...
                        # Ensure UTF-8 for writing Nepali text (Devanagari)
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                final_record = {
                                    ...
                                    "text": annotated_text, # Contains final Nepali text with tags
                                }
                ...
                if 'inference_signal' in locals(): del inference_signal
...
if __name__ == "__main__":
    # --- *** Configuration for Nepali Data *** ---
    # >>> IMPORTANT: CHANGE THESE PATHS TO YOUR NEPALI DATA LOCATION <<<
    # Example assumes Nepali data uses 'nep' or 'npi' code or similar structure
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/nepali/np"  # Example: /data/nepali_corpus/nep
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/nepali/np/nep_annotated_output.jsonl" # Example: /data/nepali_corpus/nep/annotated_nepali.jsonl
    ...
    print("Starting Nepali Audio Processing and Annotation Workflow...")
