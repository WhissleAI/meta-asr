...
def remove_existing_tags(text):
    ...
    Handles Georgian accented characters properly.
    """
    preserved_tags = {
...
def fix_end_tags(text):
    ...
    Handles Georgian text with proper spacing rules.
    """
    if not isinstance(text, str):
        return text

    # Handle Georgian-specific punctuation spacing
...
def annotate_sentences(sentences):
    prompt = f'''
You are given a list of Georgian sentences that may contain existing tags like GENDER_FEMALE, EMOTION_NEU, AGE_45_60, and SPEAKER_CHANGE.

Your task is to:
1. Remove any legacy entity tags such as NER_PERSON, NER_NORP, etc.
2. Preserve these existing tags exactly as they appear.
3. **ONLY annotate entities from the following list in Georgian text.** Do NOT invent new entity types.
4. Insert entity tags in the format ENTITY_<TYPE> before each entity and append " END" after it.
5. Focus on identifying Georgian entities that match the provided list.
6. Classify the **intent** of each Georgian sentence and add an **INTENT_<INTENT_TYPE>** tag at the end.
7. Return the output as a JSON array of annotated sentences.

IMPORTANT:
- Maintain proper Georgian spacing rules (space before :;?! and no space before .,)
- Handle Georgian accented characters and special characters properly
- Follow Georgian capitalization rules for proper nouns and entities
...
entity_list_json = json.dumps([
    ...
    "STREET_NAME", "BUILDING_NUMBER", "FLOOR_NUMBER", "BUSINESS_NAME", "BRAND_NAME", "MODEL", "SERIAL_NUMBER", "IMEI", "IMSI", "DEVICE_ID",
...
Input: "მე გუშინ მარია თბილისში ვნახე. AGE_30_40 GENDER_FEMALE EMOTION_JOY SPEAKER_CHANGE"
Output: "მე გუშინ ENTITY_PERSON_NAME მარია END ENTITY_CITY თბილისში END ვნახე. AGE_30_40 GENDER_FEMALE EMOTION_JOY SPEAKER_CHANGE INTENT_INFORM"

Input:"მინდა ექიმ ბექასთან შეხვედრა პარასკევს. AGE_60_70 GENDER_MALE EMOTION_ANXIOUS"
Output:"მინდა ENTITY_DOCTOR_NAME ექიმ ბექასთან END ENTITY_APPOINTMENT_DATE პარასკევს END შეხვედრა. AGE_60_70 GENDER_MALE EMOTION_ANXIOUS INTENT_REQUEST
...
def process_jsonl_file(input_path, output_path, batch_size=10):
    """
    Process Georgian JSONL file with proper character encoding and Georgian text handling.
    """
...
if __name__ == "__main__":
    input_jsonl_path = "/external4/datasets/jsonl_data/euro/gergian_rem.jsonl"
    output_jsonl_path = "/hydra2-prev/home/compute/workspace_himanshu/Processed_Data/gemini_metadata/salvic/processed/georgian1.jsonl"
