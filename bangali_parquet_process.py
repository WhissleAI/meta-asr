...
# Note: Emotion model performance might vary across languages like Bengali.
...
def get_transcription(text_file: str) -> str:
    try:
        # *** CRITICAL: Ensure UTF-8 for Bengali script ***
...
# --- AI Annotation Functions (MODIFIED for Bengali) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    """
    Cleans up END tags and fixes spacing around tags and Bengali punctuation.
    """
    ...
    # 8. Bengali/Common punctuation spacing rules (apply last)
    # Remove space BEFORE specific punctuation (including Bengali Dari '।')
    # This regex works for Bengali '।' as it uses the same Unicode point as Devanagari Danda
    text = re.sub(r'\s+([।?!:;,.])', r'\1', text)
    # Ensure space AFTER punctuation if followed by a word character (Bengali or Latin)
    # \w includes Unicode letters, covering Bengali script characters.
...
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt adapted for Bengali)."""
    if not genai:
    ...
    # *** BENGALI PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator for **Bengali** text written in the **Bengali (Eastern Nagari)** script.
You will receive a list of Bengali sentences. Each sentence already includes metadata tags (AGE_*, GENDER_*, EMOTION_*) at the end.
...
2.  **ENTITY ANNOTATION (Bengali Text Only):** Identify entities ONLY within the Bengali transcription part of the sentence. Use ONLY the entity types from the provided list.
3.  **ENTITY TAG FORMAT (VERY IMPORTANT):**
    ...
    *   Immediately **AFTER** the Bengali entity text, add a single space followed by `END`.
    *   Example: `... ENTITY_CITY দিল্লিতে END ...` (using Bengali script for Delhi)
    *   **DO NOT** add an `END` tag just before the `AGE_`, `GENDER_`, `EMOTION_` tags unless it belongs to a preceding entity.
4.  **INTENT TAG:** Determine the single primary intent of the Bengali transcription (e.g., INFORM, QUESTION, REQUEST, COMMAND, GREETING, etc.). Add ONE `INTENT_<INTENT_TYPE>` tag at the absolute end of the entire string, AFTER all other tags.
5.  **OUTPUT FORMAT:** Return a JSON array of strings, where each string is a fully annotated sentence adhering to all rules.
6.  **BENGALI SPECIFICS:** Handle **Bengali script (Eastern Nagari)**, punctuation (like the Dari `।`), and spacing correctly according to standard Bengali rules. Ensure proper spacing around the inserted tags.
...
**Example Input String (Bengali):**
"আমি কাল মারিয়াকে দিল্লিতে দেখেছিলাম। AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL"

**CORRECT Example Output String (Bengali):**
"আমি কাল ENTITY_PERSON_NAME মারিয়া END কে ENTITY_CITY দিল্লিতে END দেখেছিলাম। AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"

**INCORRECT Example Output String (Spaces in Tag):**
"আমি কাল ENTITY_PERSON_ NAM E মারিয়া END কে ENTITY_CIT Y দিল্লিতে END দেখেছিলাম। AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"

**INCORRECT Example Output String (Extra END before metadata):**
"আমি কাল ENTITY_PERSON_NAME মারিয়া END কে ENTITY_CITY দিল্লিতে END দেখেছিলাম। END AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"


**Sentences to Annotate Now:**
{json.dumps(texts_to_annotate, ensure_ascii=False, indent=2)}
'''
    # *** BENGALI PROMPT ENDS HERE ***
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     if isinstance(sentence, str):
                          ...
                          # 2. Fix END tags and spacing (using Bengali-aware function)
...
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Bengali audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Bengali prompt), and saves final JSONL output.
    """
    ...
    try:
        ...
        # *** CRITICAL: Ensure UTF-8 for writing Bengali output ***
    ...
    # Emotion Model (Largely Language Agnostic, but monitor performance on Bengali)
    ...
    # --- Prepare File Paths (Using base_dir for Bengali data) ---
    ...
    for i, (audio_path, text_path) in enumerate(file_pairs):
        ...
        try:
            # 1. Get Bengali Transcription (Ensures UTF-8)
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                # Call Gemini for annotation (with retries and Bengali prompt)
                ...
                if len(annotated_texts) != current_batch_size:
                    ...
                    try:
                        ...
                        # *** CRITICAL: Ensure UTF-8 for appending Bengali output ***
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                ...
                                # *** CRITICAL: ensure_ascii=False for Bengali ***
...
if __name__ == "__main__":
    ...
    # 1. SET THE BASE DIRECTORY CONTAINING YOUR BENGALI 'audio' and 'text' subfolders
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/bengali/bn"  # <--- CHANGE THIS

    # 2. SET THE DESIRED OUTPUT FILENAME FOR THE ANNOTATED BENGALI DATA
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/bengali/bn/bn_annotated_data.jsonl" # <--- CHANGE THIS (e.g., bn for Bengali)
