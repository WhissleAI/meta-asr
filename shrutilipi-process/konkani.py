# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Konkani script comments/examples
import os
import gc
import json
import re
...
# Note: Emotion model performance might vary across languages like Konkani.
...
class AudioSegment:
    ...
    age: float
    gender: str
    transcription: str # This will hold Konkani text
    emotion: str
    chunk_filename: str
    duration: float
...
def get_transcription(text_file: str) -> str:
    """Reads Konkani transcription from a text file."""
    try:
        # *** CRITICAL: Ensure UTF-8 for Konkani script (e.g., Devanagari) ***
...
# --- AI Annotation Functions (MODIFIED for Konkani) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    ...
    Handles Konkani script characters (e.g., Devanagari) via \w in regex.
    """
    ...
    text = re.sub(r'\s+([?!:;,।\.])', r'\1', text) # Added Devanagari Danda
    #    Ensure space AFTER punctuation if followed by a word character (\w handles Devanagari script)
    text = re.sub(r'([?!:;,।\.])(\w)', r'\1 \2', text) # Added Devanagari Danda
...
# --- MODIFIED AND MORE DETAILED PROMPT FOR KONKANI (Devanagari Script Focus) ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for clarity and stricter rules for Konkani in Devanagari script)."""
    if not genai:
    ...
    # Input texts already have metadata like: "konkani text AGE_X GENDER_Y EMOTION_Z"

    # *** REFINED KONKANI PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator specifically for **Konkani** text written in the **Devanagari** script. Your task is to process a list of Konkani sentences, each already containing `AGE_*`, `GENDER_*`, and `EMOTION_*` metadata tags at the end. Follow these instructions with extreme precision:
...
2.  **Entity Annotation (Konkani Text Only):**
    *   Identify entities **only** within the main Konkani transcription part of the sentence (before the metadata).
...
3.  **Strict Entity Tagging Format:**
    *   **Tag Insertion:** Place the entity tag **immediately before** the identified Konkani entity text. The tag format is `ENTITY_<TYPE>` (e.g., `ENTITY_CITY`, `ENTITY_PERSON_NAME`).
    ...
    *   **`END` Tag Placement:** Place the literal string `END` **immediately after** the *complete* Konkani entity text.
    ...
    *   **Example:** For the entity "पणजी", the correct annotation is `ENTITY_CITY पणजी END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Konkani sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`).
...
6.  **Konkani Language Specifics:**
    *   Handle Konkani script (Devanagari) correctly.
...
*   **Input:** `"हांव फाल्यां रमाक मडगांवांत मेळटलों। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"हांव फाल्यां ENTITY_PERSON_NAME रमा END क ENTITY_CITY मडगांवांत END मेळटलों। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"तुमचें नांव कितें? AGE_18_30 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"तुमचें नांव कितें? AGE_18_30 GENDER_FEMALE EMOTION_NEUTRAL INTENT_QUESTION"`

*   **Input:** `"माफ करात, म्हाका पांच मिनटां वेळ जाय। AGE_45_60 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"माफ करात, म्हाका ENTITY_DURATION पांच मिनटां END वेळ जाय। AGE_45_60 GENDER_MALE EMOTION_NEUTRAL INTENT_REQUEST"`

*   **Input:** `"देव बरें करूं। AGE_60PLUS GENDER_FEMALE EMOTION_HAPPY"`
*   **Correct Output:** `"देव बरें करूं। AGE_60PLUS GENDER_FEMALE EMOTION_HAPPY INTENT_GREETING"`


**Incorrect Examples (Common Mistakes to Avoid):**
*   `... ENTITY_PERSON_ NAM E रमा END ...` (Space in TYPE)
*   `... ENTITY_CITY मडगांवांतEND ...` (Missing space before END)
*   `... ENTITY_CITY मडगांवांत ENDमेळटलों ...` (Missing space after END)
*   `... मेळटलों। END AGE_...` (Unnecessary END before metadata)
*   `... रमा END क END ...` (Incorrectly adding END after non-entity words)
...
'''
    # *** REFINED KONKANI PROMPT ENDS HERE ***
...
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Konkani audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Konkani prompt), and saves final JSONL output.
    """
    ...
    for i, (audio_path, text_path) in enumerate(file_pairs):
        ...
        try:
            # 1. Get Transcription (Ensures UTF-8 for Konkani)
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                # Use the Konkani-specific annotation function
                ...
                if len(annotated_texts) != current_batch_size:
                    ...
                    try:
                        ...
                        # Ensure UTF-8 for writing Konkani text
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                final_record = {
                                    ...
                                    "text": annotated_text, # Contains final Konkani text with tags
                                }
...
if __name__ == "__main__":
    # --- *** Configuration for Konkani Data *** ---
    # >>> IMPORTANT: CHANGE THESE PATHS TO YOUR KONKANI DATA LOCATION <<<
    # Example assumes Konkani data uses 'kok' code or similar structure
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/konkani/kk"  # Example: /data/konkani_corpus/kok
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/konkani/kk/kn_annotated_output.jsonl" # Example: /data/konkani_corpus/kok/annotated_konkani.jsonl
    ...
    print("Starting Konkani Audio Processing and Annotation Workflow...")
