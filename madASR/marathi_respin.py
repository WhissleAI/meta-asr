# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Marathi script (Devanagari) comments/examples
import os
import gc
import json
import re
...
def fix_end_tags_and_spacing(text: str) -> str:
    """
    Cleans up AI-generated annotations for Marathi text (Devanagari), focusing on END tag placement,
...
# --- *** MODIFIED PROMPT FOR MARATHI *** ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt modified for Marathi)."""
    if not genai:
    ...
    # Input texts have metadata like: "marathi text AGE_X GENDER_Y EMOTION_Z DOMAIN_W" (DOMAIN_W is already capitalized)

    # *** MODIFIED MARATHI PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator specifically for **Marathi** text written in the **Devanagari script**. Your task is to process a list of Marathi sentences, each already containing `AGE_*`, `GENDER_*`, `EMOTION_*`, and `DOMAIN_*` metadata tags at the end. Follow these instructions with extreme precision:
...
2.  **Entity Annotation (Marathi Text Only):**
    *   Identify entities **only** within the main Marathi transcription part of the sentence (before the metadata).
...
3.  **Strict Entity Tagging Format:**
    *   **Tag Insertion:** Place the entity tag **immediately before** the identified Marathi entity text. The tag format is `ENTITY_<TYPE>` (e.g., `ENTITY_CITY`, `ENTITY_PERSON_NAME`).
    ...
    *   **`END` Tag Placement:** Place the literal string `END` **immediately after** the *complete* Marathi entity text.
    ...
    *   **Example:** For the Marathi entity "पुणे", the correct annotation is `ENTITY_CITY पुणे END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Marathi sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`, `OTHER`).
...
**Examples Demonstrating Correct Formatting (Marathi - Using Devanagari script):**

*   **Input:** `"माझं नाव रमेश आहे. AGE_30-45 GENDER_MALE EMOTION_NEUTRAL DOMAIN_GENERAL"`
*   **Correct Output:** `"माझं नाव ENTITY_PERSON_NAME रमेश END आहे. AGE_30-45 GENDER_MALE EMOTION_NEUTRAL DOMAIN_GENERAL INTENT_INFORM"`

*   **Input:** `"तो पुणे मध्ये राहतो. AGE_45-60 GENDER_MALE EMOTION_NEUTRAL DOMAIN_LOCATION"`
*   **Correct Output:** `"तो ENTITY_CITY पुणे END मध्ये राहतो. AGE_45-60 GENDER_MALE EMOTION_NEUTRAL DOMAIN_LOCATION INTENT_INFORM"`

*   **Input:** `"बैठक १० मे रोजी होईल. AGE_18-24 GENDER_MALE EMOTION_NEUTRAL DOMAIN_MEETING"`
*   **Correct Output:** `"बैठक ENTITY_DATE १० मे END रोजी होईल. AGE_18-24 GENDER_MALE EMOTION_NEUTRAL DOMAIN_MEETING INTENT_INFORM"`

*   **Input:** `"कृपया शांत रहा? AGE_60+ GENDER_MALE EMOTION_ANGRY DOMAIN_INSTRUCTION"`
*   **Correct Output:** `"कृपया शांत रहा? AGE_60+ GENDER_MALE EMOTION_ANGRY DOMAIN_INSTRUCTION INTENT_REQUEST"`
...
*   `... ENTITY_CITY पुणे ENDमध्ये ...` (Missing space after END)
*   `... राहतो END AGE_...` (Unnecessary END before metadata)
*   `... १० END मे END ...` (Incorrectly adding END after parts of a multi-word entity)
...
'''
    # *** MODIFIED MARATHI PROMPT ENDS HERE ***
        ...
        try:
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     if isinstance(sentence, str): processed_sentences.append(fix_end_tags_and_spacing(sentence)) # Apply Marathi fixing rules (uses Devanagari rules)
                     else:
...
# --- *** Main Processing Function for Marathi *** ---
def process_marathi_json(input_json_path: str, output_jsonl_path: str, base_audio_path: Optional[str], batch_size: int = 10) -> None:
    """
    Processes a Marathi JSON file (assuming similar structure to original script input),
    determines age group via fallback logic (reassigned -> original -> predicted), predicts emotion,
    constructs audio paths (absolute if base provided), annotates text with Gemini (using Marathi prompt),
    capitalizes domain tag, and saves final JSONL output.

    Args:
        input_json_path: Path to the input JSON file (expected to contain Marathi data).
    ...
    try:
        print(f"Loading Marathi data from {input_json_path}...")
    ...
    print(f"Starting processing of {total_records} Marathi records...")
    ...
    for i, (record_key, record_data) in enumerate(records_to_process):
        ...
        # *** IMPORTANT: Verify these keys match your Marathi JSON structure ***
        ...
        if missing_keys:
            print(f"  Skipping: Record missing required keys: {', '.join(missing_keys)} (Verify keys in Marathi JSON!)"); skipped_records_count += 1; continue

        wav_path_relative = record_data["wav_path"]
        text = record_data["text"] # Should be Marathi text in Devanagari
        ...
        # Uses 'age_group_reassigned', 'age_group' keys - verify these exist in your Marathi JSON
        ...
        # Ensure text is clean Marathi in Devanagari before adding tags
        ...
        if len(processed_records_buffer) >= batch_size or (i + 1) == total_records:
            ...
            print(f"\n--- Annotating Batch {batch_num} ({current_batch_size} records) with Marathi Prompt ---")
            texts_to_annotate = [rec["text_for_gemini"] for rec in processed_records_buffer]
            annotated_texts = annotate_batch_texts(texts_to_annotate) # Uses Marathi prompt
                    ...
                    with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                        for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                            final_record = {
                                ...
                                "text": annotated_text, # Contains Marathi text + tags
                                "duration": record_data["duration"]
                            }
    ...
    print("Note: Ensure input JSON keys ('wav_path', 'text', 'gender_reassigned', etc.) match the script for your Marathi data.")
    print("="*30)


# --- Main Execution ---
if __name__ == "__main__":
    # --- *** Configuration for Marathi *** ---
    # !!! IMPORTANT: Update these paths for your Marathi dataset !!!
    INPUT_JSON_FILE = "/external4/datasets/MADASR/IISc_RESPIN_train_small/data/mr_meta_data.json" # e.g., /data/marathi/metadata.json
    FINAL_OUTPUT_JSONL = "/external4/datasets/MADASR/process_train_small/marathi_output.jsonl" # e.g., /data/marathi_processed/output.jsonl
    ...
    BASE_AUDIO_DIRECTORY = "/external4/datasets/MADASR/IISc_RESPIN_train_small" # e.g., "/data/marathi/wavs" or None
    PROCESSING_BATCH_SIZE = 10 # Adjust based on API limits and memory

    # --- Verifications ---
    print("--- Marathi Data Processing Script ---")
    load_dotenv()
    ...
    print("\nStarting Marathi JSON Processing with Age Fallback Logic...")
    ...
    # Call the Marathi processing function
    process_marathi_json(
    ...
    print("\nMarathi Workflow with Age Fallback complete.")
