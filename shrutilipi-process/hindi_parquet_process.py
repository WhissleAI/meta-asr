...
try:
    ...
    # Optional: Test with a simple call if needed
...
# --- Data Structures (Script 1 - unchanged) ---
@dataclass
class AudioSegment:
    start_time: float
    end_time: float
...
class ChunkData:
    ...
    def get_formatted_text(self) -> str:
        ...
        transcription = segment.transcription.strip().lower()
...
# --- File Handling (Script 1 - unchanged) ---
def get_file_pairs(audio_dir: str, text_dir: str) -> List[Tuple[str, str]]:
    try:
        audio_files_list = [f for f in os.listdir(audio_dir) if f.lower().endswith('.flac')]
...
# --- AI Annotation Functions (Script 2, MODIFIED) ---

def correct_entity_tag_spaces(text: str) -> str:
    """Removes spaces within ENTITY_TYPE names like 'CIT Y' -> 'CITY'."""
    # This pattern finds ENTITY_ followed by uppercase letters/underscores possibly separated by spaces,
    # captures the part with spaces, and replaces it with the space-removed version.
    # It looks ahead for a space marking the end of the tag type.
    if not isinstance(text, str): return text
    
    # Use a function for replacement to handle the group correctly
    def replace_spaces(match):
        # Group 1 contains the whole tag part like "ENTITY_PERSON_ NAM E"
        tag_part = match.group(1)
        # Remove spaces only within the type part, after "ENTITY_"
        type_part = tag_part[len("ENTITY_"):]
        corrected_type = type_part.replace(' ', '')
        return f"ENTITY_{corrected_type}"

    # Pattern: Find 'ENTITY_' followed by a sequence of uppercase letters, underscores, and spaces,
    # ending just before the space that separates the tag type from the entity text.
    pattern = r'\b(ENTITY_[A-Z0-9_ ]*?[A-Z0-9_])(?=\s+\S)' # Lookahead for space + non-space char
    
    corrected_text = re.sub(pattern, replace_spaces, text)

    # Fallback/alternative: simpler replacement if the above is too complex or misses cases
    # This handles simple cases like 'WORD1<space>WORD2' -> 'WORD1WORD2' within the tag part
    # May need to run multiple times if there are multiple spaces like 'W1 W2 W3'
    # for _ in range(3): # Run a few times to catch multiple spaces like A B C
    #      corrected_text = re.sub(r'\b(ENTITY_[A-Z0-9_]+)\s+([A-Z0-9_]+)\b', r'\1\2', corrected_text)
...
def fix_end_tags_and_spacing(text: str) -> str:
    """
    Cleans up END tags and fixes spacing around tags and Hindi punctuation.
    """
    if not isinstance(text, str):
        return text

    # 1. Remove spaces within Entity Type names (e.g., "CIT Y" -> "CITY")
    #    This should ideally be done *before* this function, but included here for safety.
    text = correct_entity_tag_spaces(text)

    # 2. Remove potential leading/trailing whitespace from the whole string
    text = text.strip()
    
    # 3. Normalize whitespace (replace multiple spaces/tabs/newlines with a single space)
    ...
    #    Find ENTITY tag + text not currently followed by END
    #    Pattern Breakdown:
    #    (ENTITY_[A-Z0-9_]+\s+) : Capture tag + space
    #    ((?:(?!\bENTITY_|\bAGE_|\bGENDER_|\bEMOTION_|\bINTENT_|\bEND\b)\S+\s*)+) : Capture entity text words.
    #       Negative lookahead prevents capturing other tags or existing END.
    #       \S+ matches non-space characters (the word). \s* matches trailing space. + repeats.
    #    (?!\s*END\b) : Ensure it's not already followed by END
    
    # Simpler approach for step 6: Add END if missing, relies on previous steps cleaning bad ENDs.
    # Find `ENTITY_TAG text` where `text` does not end with ` END` but is followed by space/metadata/end
    ...
    # 8. Final Hindi punctuation spacing rules (apply last)
    # Remove space BEFORE specific Hindi punctuation and common English ones
...
def annotate_batch_texts(texts_to_annotate: List[str]):
    ...
    # Input texts already have metadata like: "hindi text AGE_X GENDER_Y EMOTION_Z"
    # Prompt needs to be very clear about handling these existing tags and formatting.
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for sentence in annotated_sentences_raw:
                     if isinstance(sentence, str):
                          ...
                          processed_sentences.append(sentence) # Or handle appropriately
...
# --- Main Processing Function (Combined Logic - MODIFIED File Handling) ---
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    ...
    # --- **CRITICAL: Clear output file at the very beginning** ---
    try:
        ...
        try:
            ...
            try:
                ...
                if signal is None or len(signal) == 0:
                    ...
                    continue
                duration = round(len(signal) / sr, 2) # Round duration
                if duration < 0.1:
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                # Call Gemini for annotation (with retries handled inside)
                ...
                if len(annotated_texts) != current_batch_size:
                    ...
                    # with open(output_jsonl_path + f".error_batch_{batch_num}.log", 'w', encoding='utf-8') as f_err:
                    #     json.dump(processed_records_buffer, f_err, indent=2, ensure_ascii=False)
                    #     f_err.write("\n---ANNOTATION INPUT---\n")
                    #     json.dump(texts_to_annotate, f_err, indent=2, ensure_ascii=False)
                    #     f_err.write("\n---ANNOTATION OUTPUT---\n")
                    #     json.dump(annotated_texts, f_err, indent=2, ensure_ascii=False)
                        ...
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                ...
                                final_record = {
                                    ...
                                    # Optionally include raw predictions if desired
                                    # "raw_age_output": record_data["raw_age_output"],
                                    # "raw_gender_prediction": record_data["raw_gender_prediction"],
                                    # "raw_emotion_prediction": record_data["raw_emotion_prediction"],
...
if __name__ == "__main__":
    # --- Configuration ---
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/hindi/hn"
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/hindi/process_data/hn_annotated_final_v2.jsonl" # Use a new name
    PROCESSING_BATCH_SIZE = 5 # Keep small for testing, increase cautiously (e.g., 10, 20)

    print("Starting Audio Processing and Annotation Workflow...")
