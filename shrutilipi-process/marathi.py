...
# Note: Emotion model performance might vary across languages like Marathi.
...
class AudioSegment:
    ...
    age: float
    gender: str
    transcription: str # This will hold Marathi text
    emotion: str
    chunk_filename: str
    duration: float
...
def get_transcription(text_file: str) -> str:
    """Reads Marathi transcription from a text file."""
    try:
        # *** CRITICAL: Ensure UTF-8 for Marathi script (Devanagari) ***
...
# --- AI Annotation Functions (MODIFIED for Marathi) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    ...
    spacing around tags, internal spaces in tag types, and Marathi punctuation.
    ...
    #    Handle cases like "ENDचे" -> "END चे"
    ...
    # 8. Marathi/Common punctuation spacing rules
    #    Remove space BEFORE specific punctuation
    text = re.sub(r'\s+([?!:;,.।])', r'\1', text) # Added Marathi danda ।
    #    Ensure space AFTER punctuation if followed by a word character (\w handles Devanagari)
    text = re.sub(r'([?!:;,.।])(\w)', r'\1 \2', text) # Added Marathi danda ।
...
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for clarity and stricter rules for Marathi)."""
    if not genai:
    ...
    # Input texts already have metadata like: "marathi text AGE_X GENDER_Y EMOTION_Z"
...
3.  **Strict Entity Tagging Format:**
    ...
    *   **Example:** For the entity "पुण्यात", the correct annotation is `ENTITY_CITY पुण्यात END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Marathi sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`).
...
6.  **Marathi Language Specifics:**
    ...
    *   Ensure correct spacing around Marathi punctuation (like `.`, `?`, `!`). Remove space before punctuation, ensure space after punctuation if followed by a word.
...
*   **Input:** `"मी उद्या मारियाला पुण्यात भेटेन. AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"मी उद्या ENTITY_PERSON_NAME मारिया END ला ENTITY_CITY पुण्यात END भेटेन. AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"सीबाआयचे हे सर्व युक्तीवाद मद्रास न्यायालयाचे न्यायाधीश प्रकाश यांनी फेटाळले आहेत. AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ENTITY_ORGANIZATION सीबाआय END चे हे सर्व युक्तीवाद ENTITY_LOCATION मद्रास END न्यायालयाचे न्यायाधीश ENTITY_PERSON_NAME प्रकाश END यांनी फेटाळle आहेत. AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note: Assuming "मद्रास" is the location and "प्रकाश" is the person's name. Also note the space after each `END`)*
...
*   **Input:** `"तुम्ही कृपया शांत बसाल का? AGE_45_60 GENDER_FEMALE EMOTION_ANNOYED"`
*   **Correct Output:** `"तुम्ही कृपया शांत बसाल का? AGE_45_60 GENDER_FEMALE EMOTION_ANNOYED INTENT_REQUEST"`

**Incorrect Examples (Common Mistakes to Avoid):**
*   `... ENTITY_PERSON_ NAM E मारिया END ...` (Space in TYPE)
*   `... ENTITY_CITY पुण्यातEND ...` (Missing space before END)
*   `... ENTITY_CITY पुण्यात ENDला ...` (Missing space after END)
*   `... आहेत END AGE_...` (Unnecessary END before metadata)
*   `... न्यायाधीश END प्रकाश END ...` (Incorrectly adding END after non-entity words)
...
# --- Main Processing Function (Unchanged except calls to modified functions) ---
...
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Marathi audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Marathi prompt), and saves final JSONL output.
    """
    ...
    # --- *** Prepare File Paths (THIS SECTION WAS MISSING) *** ---
    ...
    file_pairs = get_file_pairs(audio_dir, text_dir) # Now audio_dir and text_dir are defined
    if not file_pairs:
...
if __name__ == "__main__":
    ...
    print("Starting Marathi Audio Processing and Annotation Workflow (v2)...")
