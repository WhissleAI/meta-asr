# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Telugu script comments/examples
import os
import gc
import json
import re
...
# --- Age/Gender Model Definition (Unchanged - Language Agnostic) ---
...
# --- Emotion Extraction Function (Unchanged - Assumed Language Agnostic) ---
...
# --- Data Structures (Unchanged - Language Agnostic) ---
@dataclass
class AudioSegment:
    start_time: float; end_time: float; speaker: str; age: float; gender: str
    transcription: str # This will hold Telugu text
...
class ChunkData:
    ...
    def get_age_bucket(age: float) -> str:
        actual_age = round(age * 100); age_brackets = [
...
# --- AI Annotation Functions (MODIFIED for Telugu) ---
...
def correct_entity_tag_spaces(text: str) -> str:
    ...
    def replace_spaces(match):
        ...
        corrected_type = type_part.replace(' ', ''); return f"ENTITY_{corrected_type}"
    pattern = r'\b(ENTITY_[A-Z0-9_ ]*?[A-Z0-9_])(?=\s+\S)'
    corrected_text = re.sub(pattern, replace_spaces, text); return corrected_text

def fix_end_tags_and_spacing(text: str) -> str:
    """
    Cleans up AI-generated annotations for Telugu text, focusing on END tag placement,
    spacing around tags, internal spaces in tag types, and standard punctuation.
    Handles Telugu script characters via \w in regex.
    """
    if not isinstance(text, str): return text
    ...
    # 2. Fix spaces *within* ENTITY_TYPE names
    ...
    # 3. Ensure space *after* ENTITY_TAG
    text = re.sub(r'(\bENTITY_[A-Z0-9_]+)(\S)', r'\1 \2', text)

    # 4. Ensure space *before* END tag
    text = re.sub(r'(\S)(END\b)', r'\1 \2', text)

    # 5. Ensure space *after* END tag
    text = re.sub(r'(\bEND)(\S)', r'\1 \2', text)

    # 6. Remove duplicate END tags
    text = re.sub(r'\bEND\s+END\b', 'END', text)
    text = re.sub(r'\bEND\s+END\b', 'END', text) # Run twice
    ...
    # 8. Standard punctuation spacing rules (No Telugu-specific punctuation like Danda)
    #    Remove space BEFORE specific punctuation
    text = re.sub(r'\s+([?!:;.,])', r'\1', text) # Removed Devanagari Danda '।'
    #    Ensure space AFTER punctuation if followed by a word character (\w handles Telugu script)
    text = re.sub(r'([?!:;.,])(\w)', r'\1 \2', text) # Removed Devanagari Danda '।'
...
# --- MODIFIED AND MORE DETAILED PROMPT FOR TELUGU ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for clarity and stricter rules for Telugu)."""
    if not genai:
    ...
    # Input texts already have metadata like: "telugu text AGE_X GENDER_Y EMOTION_Z"

    # *** REFINED TELUGU PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator specifically for **Telugu** text written in the **Telugu script**. Your task is to process a list of Telugu sentences, each already containing `AGE_*`, `GENDER_*`, and `EMOTION_*` metadata tags at the end. Follow these instructions with extreme precision:
...
2.  **Entity Annotation (Telugu Text Only):**
    *   Identify entities **only** within the main Telugu transcription part of the sentence (before the metadata).
...
3.  **Strict Entity Tagging Format:**
    *   **Tag Insertion:** Place the entity tag **immediately before** the identified Telugu entity text. The tag format is `ENTITY_<TYPE>` (e.g., `ENTITY_CITY`, `ENTITY_PERSON_NAME`).
    ...
    *   **`END` Tag Placement:** Place the literal string `END` **immediately after** the *complete* Telugu entity text.
    ...
    *   **Example:** For the entity "హైదరాబాద్", the correct annotation is `ENTITY_CITY హైదరాబాద్ END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Telugu sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`).
...
6.  **Telugu Language Specifics:**
    *   Handle Telugu script correctly.
...
**Examples Demonstrating Correct Formatting (Telugu):**

*   **Input:** `"నా పేరు రమేష్. AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"నా పేరు ENTITY_PERSON_NAME రమేష్ END. AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"ఆమె హైదరాబాద్ లో నివసిస్తున్నారు. AGE_45_60 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ఆమె ENTITY_CITY హైదరాబాద్ END లో నివసిస్తున్నారు. AGE_45_60 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note the space after each `END` tag)*

*   **Input:** `"సమావేశం మే 10న జరుగుతుంది. AGE_18_30 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"సమావేశం ENTITY_DATE మే 10 END న జరుగుతుంది. AGE_18_30 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"దయచేసి నిశ్శబ్దంగా ఉండండి? AGE_60PLUS GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"దయచేసి నిశ్శబ్దంగా ఉండండి? AGE_60PLUS GENDER_MALE EMOTION_NEUTRAL INTENT_REQUEST"`

**Incorrect Examples (Common Mistakes to Avoid):**
*   `... ENTITY_PERSON_ NAM E రమేష్ END ...` (Space in TYPE)
*   `... ENTITY_CITY హైదరాబాద్END ...` (Missing space before END)
*   `... ENTITY_CITY హైదరాబాద్ ENDలో ...` (Missing space after END)
*   `... నివసిస్తున్నారు END AGE_...` (Unnecessary END before metadata)
*   `... మే END 10 END ...` (Incorrectly adding END after parts of a multi-word entity)
...
'''
    # *** REFINED TELUGU PROMPT ENDS HERE ***
        ...
        try:
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     if isinstance(sentence, str): processed_sentences.append(fix_end_tags_and_spacing(sentence)) # Apply Telugu fixing rules
                     else:
...
# --- Main Processing Function (Handles Telugu data structure) ---
def process_audio_and_annotate(input_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Telugu audio files from a single directory containing .wav files and data.json,
    extracts metadata, gets transcriptions from data.json, formats text,
    annotates with AI (using Telugu prompt), and saves final JSONL output.
    """
    ...
    json_path = os.path.join(input_dir, "data.json"); transcriptions = {}
    if not os.path.exists(json_path): print(f"Error: data.json not found in input directory: {input_dir}"); return
    try:
        print(f"Loading Telugu transcriptions from {json_path}...")
        with open(json_path, 'r', encoding='utf-8') as f: all_data_list = json.load(f) # Expecting a list of dicts
        for entry in all_data_list:
            if isinstance(entry, dict) and 'audioFilename' in entry and 'text' in entry: transcriptions[entry['audioFilename']] = entry['text']
            else: print(f"Warning: Skipping invalid or incomplete entry in data.json: {str(entry)[:100]}...")
        if not transcriptions: print(f"Error: No valid transcriptions found in {json_path}. Check format."); return
        print(f"Loaded {len(transcriptions)} transcriptions from data.json.")
    except json.JSONDecodeError as e: print(f"Error reading or parsing data.json: {e}"); return
    except Exception as e: print(f"An unexpected error occurred loading data.json: {e}"); return

    # --- Find Audio Files ---
    try:
        print(f"Scanning for .wav files in: {input_dir}")
        wav_files = [f for f in os.listdir(input_dir) if f.lower().endswith('.wav')]
        if not wav_files: print(f"No .wav files found in {input_dir}. Exiting."); return
        total_files = len(wav_files); print(f"Found {total_files} .wav files to process.")
    except FileNotFoundError: print(f"Error: Input directory not found: {input_dir}"); return
    except Exception as e: print(f"Error listing files in {input_dir}: {e}"); return
    print(f"Output will be saved to: {output_jsonl_path}"); print("-" * 30)

    processed_records_buffer = []; batch_num = 0
    files_processed_count = 0; total_records_saved = 0; skipped_files_count = 0
    ...
    for i, wav_filename in enumerate(wav_files):
        ...
        try:
            # 1. Find Transcription (Using same logic: strip prefix or use full name)
            transcription = None; lookup_key_used = None
            base_filename_match = re.match(r'\d+_\d+_(.*\.wav)', wav_filename, re.IGNORECASE)
            if base_filename_match:
                potential_key = base_filename_match.group(1)
                if potential_key in transcriptions: transcription = transcriptions[potential_key]; lookup_key_used = potential_key
            if transcription is None and wav_filename in transcriptions:
                transcription = transcriptions[wav_filename]; lookup_key_used = wav_filename
            if transcription is None: print(f"  Skipping: No transcription found in data.json for {wav_filename}."); skipped_files_count += 1; continue
            if not transcription.strip(): print(f"  Skipping: Empty transcription found for key '{lookup_key_used}'."); skipped_files_count += 1; continue
            ...
            try:
                ...
                if sr_orig != target_sr:
                    ...
                    if signal.ndim > 1: signal = np.mean(signal, axis=1); sr = sr_orig
                if signal is None or len(signal) == 0: print(f"  Skipping: Failed to load or empty audio {audio_path}"); skipped_files_count += 1; continue
                duration = round(len(signal) / sr, 2)
                if duration < 0.1: print(f"  Skipping: Audio too short ({duration:.2f}s) {audio_path}"); skipped_files_count += 1; continue
            except Exception as load_err: print(f"  Skipping: Error loading audio {audio_path}: {load_err}"); skipped_files_count += 1; continue

            # 3. Extract Age/Gender (Unchanged)
            age, gender = -1.0, "ERROR"; max_len_inference = 30 * sr
            try:
                ...
                gender_map = {0: "FEMALE", 1: "MALE", 2: "OTHER"}; gender = gender_map.get(gender_idx, "UNKNOWN")
            except RuntimeError as e:
                 if "CUDA out of memory" in str(e): print(f"  CUDA OOM Error (Age/Gender). Skipping."); torch.cuda.empty_cache(); gc.collect(); skipped_files_count += 1; continue
            ...
            # 5. Create Initial Record (Unchanged, assumes speaker ID from prefix)
            try: speaker = os.path.splitext(wav_filename)[0].split('_')[0]
            ...
            # 6. Annotate and Save in Batches (Uses Telugu annotation function)
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                annotated_texts = annotate_batch_texts(texts_to_annotate) # Uses Telugu-specific func
                    ...
                    try:
                        ...
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out: # Ensure UTF-8
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                ...
                                if "ANNOTATION_ERROR" in annotated_text: print(f"  Warning: Saving record for {os.path.basename(record_data['audio_filepath'])} with error flag: {annotated_text.split()[-1]}")
            ...
            import traceback; traceback.print_exc()
            torch.cuda.empty_cache(); gc.collect(); skipped_files_count += 1
            continue # Try the next file

    print("\n" + "="*30); print("Processing Finished (Telugu).")
...
if __name__ == "__main__":
    # --- *** Configuration for Telugu Data *** ---
    # >>> IMPORTANT: CHANGE THIS PATH TO YOUR TELUGU DATA DIRECTORY <<<
    # This directory should contain your .wav files AND the data.json file
    INPUT_DATA_DIR = "/external4/datasets/madasr/telugu-ekstep"  # Example: /data/telugu_corpus/asr_set_1
    # >>> IMPORTANT: SET YOUR DESIRED OUTPUT FILE PATH <<<
    FINAL_OUTPUT_JSONL = "/external4/datasets/madasr/ekstep-jsonl/annotated_telugu_output.jsonl" # Example: /data/telugu_corpus/asr_set_1/annotated_output.jsonl
    ...
    print("Starting Telugu Audio Processing and Annotation Workflow...")
    ...
    # Start the main processing function for Telugu
    ...
    print("Telugu Workflow complete.")