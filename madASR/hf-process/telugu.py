# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Telugu script
import os
import gc
import json
import re
...
class ModelHead(nn.Module):
    ...
    def forward(self, features, **kwargs):
        x = features; x = self.dropout(x); x = self.dense(x); x = torch.tanh(x); x = self.dropout(x); x = self.out_proj(x); return x
...
class AudioSegment:
    ...
    transcription: str # From JSON metadata (Telugu text)
...
class ChunkData:
    ...
    def get_formatted_text(self) -> str:
        ...
        transcription = segment.transcription.strip() # Telugu text
...
def get_metadata_info(metadata_path: str) -> Tuple[Optional[str], Optional[str]]:
    # ... (same logic as before, uses utf-8 for Telugu script)
    try:
...
# --- AI Annotation Functions (MODIFIED PROMPT and fix_end_tags for Telugu) ---
...
# *** MODIFIED fix_end_tags_and_spacing FOR TELUGU PUNCTUATION ***
def fix_end_tags_and_spacing(text: str) -> str:
    """
    Cleans up AI-generated annotations for Telugu, focusing on END tag placement,
    spacing around tags, internal spaces in tag types, and standard punctuation.
    Handles Telugu script characters via \w in regex. REMOVED Devanagari danda.
    """
    if not isinstance(text, str): return text
    ...
    text = re.sub(r'\bEND\s*(\b(?:AGE_|GENDER_|EMOTION_|INTENT_)|$)', r'\1', text)
    # Use standard punctuation for Telugu (Removed ।)
    text = re.sub(r'\s+([?!:;,.])', r'\1', text)
    # Ensure space AFTER standard punctuation if followed by a word character (\w handles Telugu script)
...
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for Telugu)."""
    if not genai:
    ...
    # Input texts have format: "telugu text AGE_X GENDER_Y EMOTION_Z"
...
2.  **Entity Annotation (Telugu Text Only):**
    *   Identify entities **only** within the main Telugu transcription part (before the metadata).
    *   Use **only** the entity types provided in the `Allowed ENTITY TYPES` list below.

3.  **Strict Entity Tagging Format:**
    *   **Tag Insertion:** `ENTITY_<TYPE>` tag immediately before the entity text.
    *   **NO SPACES IN TYPE:** Ensure `<TYPE>` has no spaces (e.g., `PERSON_NAME`).
    *   **`END` Tag Placement:** Literal string `END` immediately after the *complete* entity text.
    *   **Spacing:** Exactly one space between `ENTITY_<TYPE>` and entity start. Exactly one space between entity end and `END`. Exactly one space after `END` before the next word (unless punctuation follows).
    *   **Example:** For "హైదరాబాద్", correct annotation is `ENTITY_CITY హైదరాబాద్ END`.
...
5.  **Output Format:**
    *   Return a **JSON array** of strings, each a fully annotated sentence.

6.  **Telugu Language Specifics:**
    *   Handle Telugu script correctly.
    *   Ensure correct spacing around standard punctuation (like `.`, `?`, `!`, `,`). Remove space before, ensure space after if followed by a word.
    *   Clean output with single spaces as per rules.
...
*   **Input:** `"నేను రేపు మరియాని హైదరాబాద్‌లో కలుస్తాను. AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"నేను రేపు ENTITY_PERSON_NAME మరియా END ని ENTITY_CITY హైదరాబాద్ END లో కలుస్తాను. AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"సిబిఐ యొక్క ఈ వాదనలన్నింటినీ మద్రాస్ కోర్టు న్యాయమూర్తి ప్రకాష్ తోసిపుచ్చారు. AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ENTITY_ORGANIZATION సిబిఐ END యొక్క ఈ వాదనలన్నింటినీ ENTITY_LOCATION మద్రాస్ END కోర్టు న్యాయమూర్తి ENTITY_PERSON_NAME ప్రకాష్ END తోసిపుచ్చారు. AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note: Assuming "మద్రాస్" is location, "ప్రకాష్" is name. Note space after each `END` unless followed by punctuation like `.`)*

*   **Input:** `"నా పుట్టినరోజు డిసెంబర్ 12న. AGE_18_30 GENDER_MALE EMOTION_HAPPY"`
*   **Correct Output:** `"నా పుట్టినరోజు ENTITY_DATE డిసెంబర్ 12 END న. AGE_18_30 GENDER_MALE EMOTION_HAPPY INTENT_INFORM"`

*   **Input:** `"మీరు దయచేసి నిశ్శబ్దంగా ఉంటారా? AGE_45_60 GENDER_FEMALE EMOTION_ANNOYED"`
*   **Correct Output:** `"మీరు దయచేసి నిశ్శబ్దంగా ఉంటారా? AGE_45_60 GENDER_FEMALE EMOTION_ANNOYED INTENT_REQUEST"`

**Incorrect Examples (Common Mistakes to Avoid):**
*   `... ENTITY_PERSON_ NAM E మరియా END ...` (Space in TYPE)
*   `... ENTITY_CITY హైదరాబాద్END ...` (Missing space before END)
*   `... ENTITY_CITY హైదరాబాద్ ENDలో ...` (Missing space after END)
*   `... కలుస్తాను END AGE_...` (Unnecessary END before metadata)
*   `... న్యాయమూర్తి END ప్రకాష్ END ...` (Adding END after non-entity words)
        ...
        try:
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     # Use the MODIFIED fix function for Telugu
...
# --- Main Processing Function (Updated Comments for Telugu) ---
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Telugu audio files, extracts metadata (transcript, gender) from JSON,
    predicts age/emotion, formats text, annotates with AI (using Telugu prompt),
    ...
    if not os.path.exists(audio_dir) or not os.path.exists(metadata_dir):
        ...
        print("Please ensure your Telugu data follows the structure:")
        print(f"{base_dir}/\n  ├── audio/\n  └── metadata/")
        return

    print(f"Processing Telugu files from:\n  Audio: {audio_dir}\n  Metadata: {metadata_dir}") # Updated print
    ...
    # --- Process Files (Main loop - logic unchanged, handles Telugu data) ---
    for i, (audio_path, metadata_path) in enumerate(file_pairs):
        print(f"\nProcessing file {i+1}/{total_files}: {os.path.basename(audio_path)}")
        try:
            # 1. Get Transcription and Gender from Metadata (Handles Telugu script via UTF-8)
            ...
            # 4. Create Initial Record (Uses Telugu transcript/gender from metadata)
            ...
            initial_formatted_text = chunk.get_formatted_text() # Formats with Telugu text
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                annotated_texts = annotate_batch_texts(texts_to_annotate) # Calls Telugu-specific prompt
                    ...
                    try:
                        ...
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                ...
                                # ensure_ascii=False needed for Telugu script
...
if __name__ == "__main__":
    # --- *** Configuration for Telugu Data (Vaani-like Structure) *** ---
    # ---> !! IMPORTANT: UPDATE THESE PATHS TO YOUR TELUGU DATASET !! <---
    #      It should contain the 'audio' and 'metadata' subdirectories.
    BASE_AUDIO_META_DIR = "/external4/datasets/madasr/Vaani-transcription-part/Telugu/tl" # UPDATE THIS PATH (use 'te' or similar code)
    FINAL_OUTPUT_JSONL = "/external4/datasets/madasr/Vaani-transcription-part/Telugu/tl/te_vaani_annotated.jsonl" # UPDATE THIS PATH
    ...
    print("Starting Telugu Audio Processing and Annotation Workflow...") # Updated print
