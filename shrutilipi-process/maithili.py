...
# Note: Emotion model performance might vary across languages like Maithili.
...
def get_transcription(text_file: str) -> str:
    """Reads Maithili transcription from a text file."""
    try:
        # *** CRITICAL: Ensure UTF-8 for Maithili script (Devanagari) ***
...
# --- AI Annotation Functions (MODIFIED for Maithili) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    ...
    spacing around tags, internal spaces in tag types, and Maithili punctuation.
    ...
    # 8. Maithili/Common punctuation spacing rules
...
# --- MODIFIED AND MORE DETAILED PROMPT FOR MAITHILI ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for clarity and stricter rules for Maithili)."""
    if not genai:
    ...
    # Input texts already have metadata like: "maithili text AGE_X GENDER_Y EMOTION_Z"
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     if isinstance(sentence, str):
                          final_sentence = fix_end_tags_and_spacing(sentence) # Apply Maithili fixing rules
...
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Maithili audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Maithili prompt), and saves final JSONL output.
    """
    ...
    for i, (audio_path, text_path) in enumerate(file_pairs):
        ...
        try:
            # 1. Get Transcription (Ensures UTF-8 for Maithili/Devanagari)
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                # Use the Maithili-specific annotation function
                ...
                if len(annotated_texts) != current_batch_size:
                    ...
                    try:
                        ...
                        # Ensure UTF-8 for writing Maithili text (Devanagari)
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                final_record = {
                                    ...
                                    "text": annotated_text, # Contains final Maithili text with tags
                                }
...
if __name__ == "__main__":
    ...
    # >>> IMPORTANT: CHANGE THESE PATHS TO YOUR MAITHILI DATA LOCATION <<<
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/maithili/mt"  # Example: /data/maithili_corpus/mai
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/maithili/mt/mt_annotated_output.jsonl" # Example: /data/maithili_corpus/mai/annotated_maithili.jsonl
