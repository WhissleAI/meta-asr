...
# Note: Emotion model performance might vary across languages like Kannada.
...
class AudioSegment:
    ...
    age: float
    gender: str
    transcription: str # This will hold Kannada text
    emotion: str
    chunk_filename: str
    duration: float
...
def get_transcription(text_file: str) -> str:
    """Reads Kannada transcription from a text file."""
    try:
        # *** CRITICAL: Ensure UTF-8 for Kannada script ***
...
# --- AI Annotation Functions (MODIFIED for Kannada) ---
...
# --- MODIFIED AND MORE DETAILED PROMPT FOR KANNADA ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    ...
    # *** REFINED KANNADA PROMPT STARTS HERE ***
...
# --- Main Processing Function (Calls modified functions) ---
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Kannada audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Kannada prompt), and saves final JSONL output.
    """
        ...
        try:
            ...
            try:
                try:
                    ...
                    # Ensure mono=False to handle potential stereo, let librosa average later
                    signal_raw, sr_orig = librosa.load(audio_path, sr=None, mono=False) # Keep original channels if librosa used
                ...
                # Handle potential multichannel audio -> Convert to mono
                ...
                if sr_orig != target_sr:
                    ...
                    signal = signal_mono # Use the (potentially averaged) mono signal
                    sr = sr_orig
            ...
            try:
                ...
                max_len_inference = 30 * sr # Limit inference length for stability
                if input_values.shape[1] > max_len_inference:
                    # print(f"  Truncating long audio ({input_values.shape[1]/sr:.1f}s) to {max_len_inference/sr:.1f}s for Age/Gender/Emotion.")
            ...
            emotion = "ERROR"
            try:
                 # Use the potentially truncated signal for emotion too
                 emotion_signal = signal[:max_len_inference] if len(signal) > max_len_inference else signal
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                annotated_texts = annotate_batch_texts(texts_to_annotate) # Calls the Kannada-specific prompt
                        ...
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                final_record = {
                                    ...
                                    "text": annotated_text, # This now contains the AI's Kannada annotations
                                }
                ...
                # Explicitly delete tensors and collect garbage
            ...
            torch.cuda.empty_cache(); gc.collect() # Attempt cleanup even on error
...
if __name__ == "__main__":
    # --- *** Configuration for Kannada Data *** ---
    # ---> !! IMPORTANT: UPDATE THESE PATHS TO YOUR KANNADA DATASET !! <---
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/kannada/kd" # Example structure
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/kannada/kd/kn_annotated.jsonl" # Use a Kannada-specific name
    PROCESSING_BATCH_SIZE = 10 # Adjust based on API limits, memory, and sentence complexity (start smaller if needed)
    ...
    if not os.getenv("GOOGLE_API_KEY"):
        ...
        exit(1)
    # Re-check genai configuration in case it failed initially
    if genai is None:
        try:
    ...
    print("Starting Kannada Audio Processing and Annotation Workflow...")
