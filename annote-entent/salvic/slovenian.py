...
def remove_existing_tags(text):
    ...
    Handles Slovenian accented characters properly.
    """
    preserved_tags = {
...
def fix_end_tags(text):
    ...
    Handles Slovenian text with proper spacing rules.
    """
    if not isinstance(text, str):
        return text

    # Handle Slovenian-specific punctuation spacing
...
def annotate_sentences(sentences):
    prompt = f'''
You are given a list of Slovenian sentences that may contain existing tags like GENDER_FEMALE, EMOTION_NEU, AGE_45_60, and SPEAKER_CHANGE.

Your task is to:
1. Remove any legacy entity tags such as NER_PERSON, NER_NORP, etc.
2. Preserve these existing tags exactly as they appear.
3. **ONLY annotate entities from the following list in Slovenian text.** Do NOT invent new entity types.
4. Insert entity tags in the format ENTITY_<TYPE> before each entity and append " END" after it.
5. Focus on identifying Slovenian entities that match the provided list.
6. Classify the **intent** of each Slovenian sentence and add an **INTENT_<INTENT_TYPE>** tag at the end.
7. Return the output as a JSON array of annotated sentences.

IMPORTANT:
- Maintain proper Slovenian spacing rules (space before :;?! and no space before .,)
- Handle Slovenian accented characters and special characters properly
- Follow Slovenian capitalization rules for proper nouns and entities
...
Input: "Videl sem Marijo v Ljubljani včeraj. AGE_30_40 GENDER_FEMALE EMOTION_JOY SPEAKER_CHANGE"
Output: "Videl sem ENTITY_PERSON_NAME Marijo END v ENTITY_CITY Ljubljani END včeraj. AGE_30_40 GENDER_FEMALE EMOTION_JOY SPEAKER_CHANGE INTENT_INFORM"
...
def process_jsonl_file(input_path, output_path, batch_size=10):
    """
    Process Slovenian JSONL file with proper character encoding and Slovenian text handling.
    """
...
if __name__ == "__main__":
    input_jsonl_path = "/hydra2-prev/home/compute/workspace_himanshu/Processed_Data/gemini_metadata/salvic/slovian_uni.jsonl"
    output_jsonl_path = "/hydra2-prev/home/compute/workspace_himanshu/Processed_Data/gemini_metadata/salvic/processed/slovenian.jsonl"
