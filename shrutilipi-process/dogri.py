# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Dogri script (Devanagari) comments/examples
import os
import gc
import json
import re
...
# Note: Emotion model performance might vary across languages like Dogri.
...
class AudioSegment:
    ...
    age: float
    gender: str
    transcription: str # This will hold Dogri text
    emotion: str
    chunk_filename: str
    duration: float
...
def get_transcription(text_file: str) -> str:
    """Reads Dogri transcription from a text file."""
    try:
        # *** CRITICAL: Ensure UTF-8 for Dogri script (Devanagari) ***
...
# --- AI Annotation Functions (MODIFIED for Dogri) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    ...
    spacing around tags, internal spaces in tag types, and Dogri punctuation.
    ...
    # 8. Dogri/Common punctuation spacing rules
...
# --- MODIFIED AND MORE DETAILED PROMPT FOR DOGRI ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for clarity and stricter rules for Dogri)."""
    if not genai:
    ...
    # Input texts already have metadata like: "dogri text AGE_X GENDER_Y EMOTION_Z"

    # *** REFINED DOGRI PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator specifically for **Dogri** text written in the **Devanagari** script. Your task is to process a list of Dogri sentences, each already containing `AGE_*`, `GENDER_*`, and `EMOTION_*` metadata tags at the end. Follow these instructions with extreme precision:
...
2.  **Entity Annotation (Dogri Text Only):**
    *   Identify entities **only** within the main Dogri transcription part of the sentence (before the metadata).
...
3.  **Strict Entity Tagging Format:**
    *   **Tag Insertion:** Place the entity tag **immediately before** the identified Dogri entity text. The tag format is `ENTITY_<TYPE>` (e.g., `ENTITY_CITY`, `ENTITY_PERSON_NAME`).
    ...
    *   **`END` Tag Placement:** Place the literal string `END` **immediately after** the *complete* Dogri entity text.
    ...
    *   **Example:** For the entity "जम्मू", the correct annotation is `ENTITY_CITY जम्मू END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Dogri sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`).
...
6.  **Dogri Language Specifics:**
    *   Handle Devanagari script correctly.
    *   Ensure correct spacing around Dogri punctuation (like `।`, `.`, `?`, `!`). Remove space before punctuation, ensure space after punctuation if followed by a word.
...
*   **Input:** `"मेरा नां रमेश ऐ। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"मेरा नां ENTITY_PERSON_NAME रमेश END ऐ। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"ओ जम्मू च रैंह्दे न। AGE_45_60 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ओ ENTITY_CITY जम्मू END च रैंह्दे न। AGE_45_60 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note the space after each `END` tag)*

*   **Input:** `"मीटिंग १० मई गी होग। AGE_18_30 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"मीटिंग ENTITY_DATE १० मई END गी होग। AGE_18_30 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"कृपा करियै शांत रौह्? AGE_60PLUS GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"कृपा करियै शांत रौह्? AGE_60PLUS GENDER_MALE EMOTION_NEUTRAL INTENT_REQUEST"`

**Incorrect Examples (Common Mistakes to Avoid):**
*   `... ENTITY_PERSON_ NAM E रमेश END ...` (Space in TYPE)
*   `... ENTITY_CITY जम्मूEND ...` (Missing space before END)
*   `... ENTITY_CITY जम्मू ENDच ...` (Missing space after END)
*   `... न END AGE_...` (Unnecessary END before metadata)
...
'''
    # *** REFINED DOGRI PROMPT ENDS HERE ***
        ...
        try:
            ...
            if json_match:
                ...
                if assistant_reply.startswith('[') and assistant_reply.endswith(']'):
                     ...
                     # print("Debug: Extracted JSON string after markdown removal")
                else:
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     if isinstance(sentence, str):
                          final_sentence = fix_end_tags_and_spacing(sentence) # Apply Dogri fixing rules
                ...
                if len(processed_sentences) == len(texts_to_annotate):
                    ...
                    if attempt == max_retries - 1:
                        ...
                        raise ValueError("Processing error lead to length mismatch.")
...
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Dogri audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Dogri prompt), and saves final JSONL output.
    """
    ...
    if not os.path.exists(audio_dir) or not os.path.exists(text_dir):
        ...
        print("Please ensure your Dogri data follows the structure:")
        print(f"{base_dir}/")
        print(f"  ├── audio/  (contains .flac/.wav/.mp3 files)")
        print(f"  └── text/   (contains .txt files with matching basenames, UTF-8 encoded, Devanagari script)")
        return

    print(f"Processing Dogri files from:\n  Audio: {audio_dir}\n  Text:  {text_dir}")
    ...
    for i, (audio_path, text_path) in enumerate(file_pairs):
        ...
        try:
            # 1. Get Transcription (Ensures UTF-8 for Dogri/Devanagari)
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                # Use the Dogri-specific annotation function
                ...
                if len(annotated_texts) != current_batch_size:
                    ...
                    try:
                        ...
                        # Ensure UTF-8 for writing Dogri text (Devanagari)
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                final_record = {
                                    ...
                                    "text": annotated_text, # Contains final Dogri text with tags
                                }
...
if __name__ == "__main__":
    # --- *** Configuration for Dogri Data *** ---
    # >>> IMPORTANT: CHANGE THESE PATHS TO YOUR DOGRI DATA LOCATION <<<
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/dogri/dg"  # Example: /data/dogri_corpus/doi (ISO 639-3 code for Dogri)
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/dogri/dg/doi_annotated_output.jsonl" # Example: /data/dogri_corpus/doi/annotated_dogri.jsonl
    ...
    print("Starting Dogri Audio Processing and Annotation Workflow...")
