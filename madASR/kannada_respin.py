...
def fix_end_tags_and_spacing(text: str) -> str:
    """
    Cleans up AI-generated annotations for Kannada text, focusing on END tag placement,
    spacing around tags, internal spaces in tag types, and common punctuation.
    Assumes standard punctuation like '.', '?', '!' are used in digital Kannada text.
    """
    if not isinstance(text, str): return text
    text = re.sub(r'\s+', ' ', text).strip()
    # Fix internal spaces within ENTITY type names
    ...
    # Ensure space after ENTITY tag and before text
    ...
    text = re.sub(r'\bEND\s+END\b', 'END', text) # Run twice for triples etc.
    # Remove END tags immediately before metadata tags or end of string
    text = re.sub(r'\bEND\s*(\b(?:AGE_|GENDER_|EMOTION_|DOMAIN_|INTENT_)|$)', r'\1', text)
    text = re.sub(r'\bEND\s*(\b(?:AGE_|GENDER_|EMOTION_|DOMAIN_|INTENT_)|$)', r'\1', text) # Run twice
    # Fix spacing around punctuation (handles common Latin punctuation)
...
# --- *** MODIFIED PROMPT FOR KANNADA *** ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt modified for Kannada)."""
    if not genai:
    ...
    # Input texts have metadata like: "kannada text AGE_X GENDER_Y EMOTION_Z DOMAIN_W" (DOMAIN_W is already capitalized)

    # *** MODIFIED KANNADA PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator specifically for **Kannada** text written in the **Kannada script**. Your task is to process a list of Kannada sentences, each already containing `AGE_*`, `GENDER_*`, `EMOTION_*`, and `DOMAIN_*` metadata tags at the end. Follow these instructions with extreme precision:
...
3.  **Strict Entity Tagging Format:**
    ...
    *   **Example:** For the Kannada entity "ಬೆಂಗಳೂರು", the correct annotation is `ENTITY_CITY ಬೆಂಗಳೂರು END`.
...
4.  **Intent Tag:**
    *   Determine the single primary intent of the Kannada sentence (e.g., `INFORM`, `QUESTION`, `REQUEST`, `COMMAND`, `GREETING`, `OTHER`).
...
**Examples Demonstrating Correct Formatting (Kannada - Using Kannada script):**

*   **Input:** `"ನನ್ನ ಹೆಸರು ರಮೇಶ್. AGE_30-45 GENDER_MALE EMOTION_NEUTRAL DOMAIN_GENERAL"`
*   **Correct Output:** `"ನನ್ನ ಹೆಸರು ENTITY_PERSON_NAME ರಮೇಶ್ END. AGE_30-45 GENDER_MALE EMOTION_NEUTRAL DOMAIN_GENERAL INTENT_INFORM"`

*   **Input:** `"ಅವನು ಬೆಂಗಳೂರಿನಲ್ಲಿ ವಾಸಿಸುತ್ತಾನೆ. AGE_45-60 GENDER_MALE EMOTION_NEUTRAL DOMAIN_LOCATION"`
*   **Correct Output:** `"ಅವನು ENTITY_CITY ಬೆಂಗಳೂರಿನಲ್ಲಿ END ವಾಸಿಸುತ್ತಾನೆ. AGE_45-60 GENDER_MALE EMOTION_NEUTRAL DOMAIN_LOCATION INTENT_INFORM"`

*   **Input:** `"ಸಭೆ ಮೇ 10 ರಂದು ನಡೆಯಲಿದೆ. AGE_18-24 GENDER_MALE EMOTION_NEUTRAL DOMAIN_MEETING"`
*   **Correct Output:** `"ಸಭೆ ENTITY_DATE ಮೇ 10 ರಂದು END ನಡೆಯಲಿದೆ. AGE_18-24 GENDER_MALE EMOTION_NEUTRAL DOMAIN_MEETING INTENT_INFORM"`

*   **Input:** `"ದಯವಿಟ್ಟು ಶಾಂತವಾಗಿರಿ? AGE_60+ GENDER_MALE EMOTION_ANGRY DOMAIN_INSTRUCTION"`
*   **Correct Output:** `"ದಯವಿಟ್ಟು ಶಾಂತವಾಗಿರಿ? AGE_60+ GENDER_MALE EMOTION_ANGRY DOMAIN_INSTRUCTION INTENT_REQUEST"`

**Incorrect Examples (Common Mistakes to Avoid):**
*   `... ENTITY_PERSON_ NAM E ರಮೇಶ್ END ...` (Space in TYPE)
*   `... ENTITY_CITY ಬೆಂಗಳೂರುEND ...` (Missing space before END)
*   `... ENTITY_CITY ಬೆಂಗಳೂರಿನಲ್ಲಿ ENDವಾಸಿಸುತ್ತಾನೆ ...` (Missing space after END)
*   `... ವಾಸಿಸುತ್ತಾನೆ END AGE_...` (Unnecessary END before metadata)
*   `... ಮೇ END 10 ರಂದು END ...` (Incorrectly adding END after parts of a multi-word entity)
...
'''
    # *** MODIFIED KANNADA PROMPT ENDS HERE ***
    ...
    for attempt in range(max_retries):
        try:
            ...
            if json_match: json_str = json_match.group(0)
                ...
                # Try cleaning up potential markdown backticks
                ...
                # Check if it now looks like a JSON list
                if assistant_reply.startswith('[') and assistant_reply.endswith(']'): json_str = assistant_reply
                else:
                    # Log the failure and attempt direct parsing as a last resort
                    ...
                    try:
                        ...
                        else: # Parsed but wasn't a list
                            raise json.JSONDecodeError("Response parsed but is not a JSON list.", assistant_reply, 0)
                    except json.JSONDecodeError: # Failed even direct parsing
                         raise json.JSONDecodeError("Response does not appear to contain a JSON list.", assistant_reply, 0) # Re-raise
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     # Apply Kannada fixing rules (uses general tag/punctuation rules)
                     if isinstance(sentence, str): processed_sentences.append(fix_end_tags_and_spacing(sentence))
                          ...
                          # Try to append original text with error flag if mapping back is possible
                          try: processed_sentences.append(fix_end_tags_and_spacing(texts_to_annotate[idx]) + " ANNOTATION_ERROR_NON_STRING")
                              ...
                              processed_sentences.append("ANNOTATION_ERROR_UNKNOWN_ORIGINAL") # Placeholder error

                # Final check on processed length
                if len(processed_sentences) == len(texts_to_annotate): return processed_sentences # Success
                    ...
                    else: raise ValueError("Processing error lead to length mismatch.") # Trigger retry if possible
            else:
            ...
            if "rate limit" in str(e).lower(): print("Rate limit likely hit."); time.sleep(retry_delay * (attempt + 1) * 5) # Longer delay for rate limits
            ...
            elif "ConnectionError" in str(e): # If genai object was None initially
...
# --- *** Main Processing Function for Kannada *** ---
def process_kannada_json(input_json_path: str, output_jsonl_path: str, base_audio_path: Optional[str], batch_size: int = 10) -> None:
    """
    Processes a Kannada JSON file (assuming similar structure to original script input),
    determines age group via fallback logic (reassigned -> original -> predicted), predicts emotion,
    constructs audio paths (absolute if base provided), annotates text with Gemini (using Kannada prompt),
    capitalizes domain tag, and saves final JSONL output.

    Args:
        input_json_path: Path to the input JSON file (expected to contain Kannada data).
    ...
    try:
        print(f"Loading Kannada data from {input_json_path}...")
    ...
    print(f"Starting processing of {total_records} Kannada records...")
    ...
    for i, (record_key, record_data) in enumerate(records_to_process):
        ...
        # *** IMPORTANT: Verify these keys match your Kannada JSON structure ***
        ...
        if missing_keys:
            print(f"  Skipping: Record missing required keys: {', '.join(missing_keys)} (Verify keys in Kannada JSON!)"); skipped_records_count += 1; continue

        wav_path_relative = record_data["wav_path"]
        text = record_data["text"] # Should be Kannada text
        gender = record_data.get("gender_reassigned", record_data.get("gender", "UNKNOWN")) # Fallback if gender_reassigned missing
        ...
        def get_audio_path(rel_path, base_path, json_path):
            if not isinstance(rel_path, str):
                 print(f"  Error: wav_path is not a string: {rel_path}"); return None
            if base_path: # Prefer base path if provided
                rel_path = rel_path.lstrip('/') # Handle potential leading slash
        ...
        def load_audio_signal(path):
            ...
            try:
                # Try soundfile first (often faster and handles more formats)
                try: sig_raw, sr_orig = sf.read(path, dtype='float32')
                except Exception: sig_raw, sr_orig = librosa.load(path, sr=None, mono=False) # Fallback to librosa
        ...
        # Uses 'age_group_reassigned', 'age_group' keys - verify these exist in your Kannada JSON
        ...
        try:
            ...
            if emotion in ["ERRORLOADINGMODEL", "NO_AUDIO", "OOM_ERROR", "RUNTIME_ERROR", "EXTRACTION_ERROR"]:
                 ...
                 emotion = "UNKNOWN" # Use UNKNOWN instead of ERROR for consistency
        ...
        # Ensure text is clean Kannada script before adding tags
        cleaned_text = text.strip() # Already checked for non-empty string

        safe_age = re.sub(r'\s+', '_', str(final_age_group).strip())
        safe_gender = re.sub(r'\s+', '_', str(gender).upper().strip())
        safe_emotion = re.sub(r'\s+', '_', str(emotion).upper().strip()) # Use the potentially updated emotion
        ...
        if len(processed_records_buffer) >= batch_size or (i + 1) == total_records:
            ...
            print(f"\n--- Annotating Batch {batch_num} ({current_batch_size} records) with Kannada Prompt ---")
            texts_to_annotate = [rec["text_for_gemini"] for rec in processed_records_buffer]
            annotated_texts = annotate_batch_texts(texts_to_annotate) # Uses Kannada prompt
            ...
            if len(annotated_texts) != current_batch_size:
                 ...
                 print("Original texts sent:")
                 # for t in texts_to_annotate: print(f"  - {t[:100]}...")
                 print("Received texts:")
                 # for a in annotated_texts: print(f"  - {a[:100]}...")
                    ...
                    with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                        for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                            final_record = {
                                ...
                                "text": annotated_text, # Contains Kannada text + tags
                                "duration": record_data["duration"]
                            }
                            # Log if Gemini returned an error flag within the text
                            ...
                            json_str = json.dumps(final_record, ensure_ascii=False) # ensure_ascii=False for Kannada script
        ...
        if 'signal' in locals() and signal is not None : del signal
        if 'audio_path' in locals(): del audio_path # audio_path is just a string, less critical
        gc.collect()
    ...
    print("Note: Ensure input JSON keys ('wav_path', 'text', 'gender_reassigned', etc.) match the script for your Kannada data.")
    print("="*30)


# --- Main Execution ---
if __name__ == "__main__":
    # --- *** Configuration for Kannada *** ---
    # !!! IMPORTANT: Update these paths for your Kannada dataset !!!
    INPUT_JSON_FILE = "/external4/datasets/MADASR/IISc_RESPIN_train_small/data/kn_meta_data.json" # e.g., /data/kannada/metadata.json
    FINAL_OUTPUT_JSONL = "/external4/datasets/MADASR/process_train_small/kannada_output.jsonl" # e.g., /data/kannada_processed/output.jsonl
    ...
    BASE_AUDIO_DIRECTORY = "/external4/datasets/MADASR/IISc_RESPIN_train_small" # e.g., "/data/kannada/wavs" or None
    PROCESSING_BATCH_SIZE = 10 # Adjust based on API limits and memory

    # --- Verifications ---
    print("--- Kannada Data Processing Script ---")
    load_dotenv()
    ...
    print("\nStarting Kannada JSON Processing with Age Fallback Logic...")
    ...
    # Call the Kannada processing function
    process_kannada_json(
    ...
    print("\nKannada Workflow with Age Fallback complete.")
