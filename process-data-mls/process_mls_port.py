...
# Initialize the Portuguese NER model and tokenizer
def initialize_portuguese_ner():
...
portuguese_ner = initialize_portuguese_ner()
...
def process_ner(text: str) -> Tuple[str, List[Dict[str, Any]]]:
    """
    Process text using Portuguese BERT NER model
    """
    try:
        # Get NER predictions
        ner_results = portuguese_ner(text)
...
def process_audio_files_with_transcriptions(base_dir: str, output_dir: str = "output", batch_size: int = 2500) -> None:
...
if __name__ == "__main__":
    base_dir = "/external2/datasets/librespeech/mls_portuguese/dev"
    output_dir = "/external2/datasets/json_jata/portuguse/dev"
