...
# Note: The actual subdirectory will be created in _process_single_downloaded_file
    ...
    try:
        ...
        return SingleFileProcessResponse(
            ...
            status_message=status_msg,
            duration=processing_results.get("duration"),
            transcription=processing_results.get("transcription")[0] if processing_results.get("transcription") else "",
            age_group=processing_results.get("age_group")[0] if processing_results.get("age_group") else "",
            gender=processing_results.get("gender")[0] if processing_results.get("gender") else "",
            emotion=processing_results.get("emotion")[0] if processing_results.get("emotion") else "",
            bio_annotation_gemini=processing_results.get("bio_annotation_gemini")[0] if processing_results.get("bio_annotation_gemini") else None,
            gemini_intent=processing_results.get("gemini_intent")[0] if processing_results.get("gemini_intent") else "",
            prompt_used=processing_results.get("prompt_used"),
            error_details=processing_results.get("error_details"),
            overall_error=processing_results.get("overall_error_summary")
        )
