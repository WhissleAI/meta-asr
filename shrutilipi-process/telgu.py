...
# Note: Emotion model performance might vary across languages like Telugu.
...
def get_transcription(text_file: str) -> str:
    """Reads Telugu transcription from a text file."""
    try:
        # *** CRITICAL: Ensure UTF-8 for Telugu script ***
...
def fix_end_tags_and_spacing(text: str) -> str:
    ...
    #    Handle cases like "ENDచె" -> "END చె"
    ...
    #    (Removed Marathi danda ।. Using common punctuation)
...
*   **Input:** `"రేపు నేను మరియాని హైదరాబాద్‌లో కలుస్తాను. AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"రేపు నేను ENTITY_PERSON_NAME మరియ END ని ENTITY_CITY హైదరాబాద్ END లో కలుస్తాను. AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"సిబిఐ యొక్క ఈ వాదనలన్నింటినీ మద్రాస్ కోర్టు న్యాయమూర్తి ప్రకాష్ తిరస్కరించారు. AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ENTITY_ORGANIZATION సిబిఐ END యొక్క ఈ వాదనలన్నింటినీ ENTITY_LOCATION మద్రాస్ END కోర్టు న్యాయమూర్తి ENTITY_PERSON_NAME ప్రకాష్ END తిరస్కరించారు. AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note: Assuming "మద్రాస్" is the location and "ప్రకాష్" is the person's name. Also note the space after each `END`)*
...
*   `... ENTITY_PERSON_ NAM E మరియ END ...` (Space in TYPE)
*   `... ENTITY_CITY హైదరాబాద్END ...` (Missing space before END)
*   `... ENTITY_CITY హైదరాబాద్ ENDలో ...` (Missing space after END)
*   `... తిరస్కరించారు END AGE_...` (Unnecessary END before metadata)
*   `... న్యాయమూర్తి END ప్రకాష్ END ...` (Incorrectly adding END after non-entity words)
...
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Telugu audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Telugu prompt), and saves final JSONL output.
    """
    ...
    for i, (audio_path, text_path) in enumerate(file_pairs):
        ...
        try:
            # 1. Get Transcription (Ensures UTF-8 for Telugu)
            ...
            try:
                ...
                if sr_orig != target_sr:
                    ...
                    # Ensure float32 if already correct SR but wrong dtype
            ...
            try:
                ...
                if input_values.shape[1] > max_len_inference:
                    ...
                    # print(f"  Truncated audio for Age/Gender inference (>{max_len_inference/sr}s).")
                else:
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                # Use the Telugu-specific annotation function
                ...
                if len(annotated_texts) != current_batch_size:
                    ...
                    try:
                        ...
                        # Ensure UTF-8 for writing Telugu text
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                final_record = {
                                    ...
                                    "text": annotated_text, # This contains the final Telugu text with tags
                                }
...
if __name__ == "__main__":
    ...
    # >>> IMPORTANT: CHANGE THESE PATHS TO YOUR TELUGU DATA LOCATION <<<
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/telugu/tl"  # Example: /data/telugu_corpus/te
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/telugu/tl/te_annotated_output.jsonl" # Example: /data/telugu_corpus/te/annotated_telugu.jsonl
    # <<< END OF PATHS TO CHANGE <<<

    PROCESSING_BATCH_SIZE = 10 # Adjust based on API limits and system memory (start small, e.g., 10)
    ...
    if genai is None:
        try:
            ...
            print("Google Generative AI re-configured successfully.")
             ...
             print("Ensure your API key is correct and the Generative Language API is enabled in your Google Cloud project.")
             exit(1)
