...
class AudioSegment:
    ...
    transcription: str # Will come from JSON metadata (Bengali text)
...
class ChunkData:
    ...
    def get_formatted_text(self) -> str:
        ...
        transcription = segment.transcription.strip() # Bengali text
...
# --- File Handling (Unchanged logic, relies on correct paths) ---
def get_file_pairs(audio_dir: str, metadata_dir: str) -> List[Tuple[str, str]]:
    ...
    try:
        ...
        return pairs
    except FileNotFoundError as e: print(f"Error finding files: {e}. Check directory paths."); return []
    except Exception as e: print(f"An unexpected error occurred in get_file_pairs: {e}"); return []


# --- Get Info from Metadata JSON (Unchanged logic, assumes same keys) ---
def get_metadata_info(metadata_path: str) -> Tuple[Optional[str], Optional[str]]:
    # ... (same logic as before, uses utf-8)
    try:
        with open(metadata_path, 'r', encoding='utf-8') as f: data = json.load(f)
        transcript = data.get("transcript")
        gender = data.get("gender")
        if not transcript: print(f"Warning: 'transcript' key missing or empty in {metadata_path}"); transcript = None
        if not gender: print(f"Warning: 'gender' key missing or empty in {metadata_path}"); gender = None
        else:
             gender = gender.upper()
             if gender not in ["FEMALE", "MALE", "OTHER"]: # Added OTHER for generality
    ...
    except Exception as e: print(f"Error reading metadata file {metadata_path}: {str(e)}"); return None, None


# --- AI Annotation Functions (MODIFIED PROMPT for Bengali) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    ...
    text = re.sub(r'\bEND\s*(\b(?:AGE_|GENDER_|EMOTION_|INTENT_)|$)', r'\1', text)
    text = re.sub(r'\s+([?!:;,.।])', r'\1', text) # Includes Bengali danda ।
    text = re.sub(r'([?!:;,.।])(\w)', r'\1 \2', text) # Includes Bengali danda ।
    text = re.sub(r'\s+', ' ', text).strip()
    return text


# --- MODIFIED AND MORE DETAILED PROMPT FOR BENGALI ---
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt refined for clarity and stricter rules for Bengali)."""
    if not genai:
...
*   **Input:** `"আমি আগামীকাল মারিয়ার সাথে কলকাতায় দেখা করব। AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"আমি আগামীকাল ENTITY_PERSON_NAME মারিয়া END র সাথে ENTITY_CITY কলকাতা END য় দেখা করব। AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"সিবিআইয়ের এই সমস্ত যুক্তি মাদ্রাজ আদালতের বিচারক প্রকাশ খারিজ করে দিয়েছেন। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ENTITY_ORGANIZATION সিবিআই END য়ের এই সমস্ত যুক্তি ENTITY_LOCATION মাদ্রাজ END আদালতের বিচারক ENTITY_PERSON_NAME প্রকাশ END খারিজ করে দিয়েছেন। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note: Assuming "মাদ্রাজ" is location, "প্রকাশ" is name. Note space after each `END` unless followed by punctuation like `।`)*

*   **Input:** `"আমার জন্মদিন ডিসেম্বর ১২ তারিখে। AGE_18_30 GENDER_MALE EMOTION_HAPPY"`
*   **Correct Output:** `"আমার জন্মদিন ENTITY_DATE ডিসেম্বর ১২ END তারিখে। AGE_18_30 GENDER_MALE EMOTION_HAPPY INTENT_INFORM"`

*   **Input:** `"আপনি দয়া করে চুপ করবেন? AGE_45_60 GENDER_FEMALE EMOTION_ANNOYED"`
*   **Correct Output:** `"আপনি দয়া করে চুপ করবেন? AGE_45_60 GENDER_FEMALE EMOTION_ANNOYED INTENT_REQUEST"`

**Incorrect Examples (Common Mistakes to Avoid):**
*   `... ENTITY_PERSON_ NAM E মারিয়া END ...` (Space in TYPE)
*   `... ENTITY_CITY কলকাতাEND ...` (Missing space before END)
*   `... ENTITY_CITY কলকাতা ENDয় ...` (Missing space after END)
*   `... দিয়েছেন END AGE_...` (Unnecessary END before metadata)
*   `... বিচারক END প্রকাশ END ...` (Incorrectly adding END after non-entity words)
...
# --- Main Processing Function (Updated Comments for Bengali) ---
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Bengali audio files, extracts metadata (transcript, gender) from JSON,
    predicts age/emotion, formats text, annotates with AI (using Bengali prompt),
        ...
        try:
            ...
            try:
                ...
                if input_values.shape[1] > max_len_inference:
                    input_values_truncated = input_values[:, :max_len_inference].to(device); emotion_signal = signal[:max_len_inference]
                else:
                    input_values_truncated = input_values.to(device); emotion_signal = signal
            ...
            # 4. Create Initial Record (Uses Bengali transcript/gender from metadata)
            ...
            initial_formatted_text = chunk.get_formatted_text() # Formats with Bengali text
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                annotated_texts = annotate_batch_texts(texts_to_annotate) # Calls Bengali-specific prompt
                    ...
                    try:
                        ...
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                ...
                                json_str = json.dumps(final_record, ensure_ascii=False) # ensure_ascii=False for Bengali
...
if __name__ == "__main__":
    # --- *** Configuration for Bengali Data (Vaani-like Structure) *** ---
    # ---> !! IMPORTANT: UPDATE THESE PATHS TO YOUR BENGALI DATASET !! <---
    #      It should contain the 'audio' and 'metadata' subdirectories.
    BASE_AUDIO_META_DIR = "/external4/datasets/madasr/Vaani-transcription-part/Bengali/bn" # UPDATE THIS PATH
    FINAL_OUTPUT_JSONL = "/external4/datasets/madasr/Vaani-transcription-part/Bengali/bn/bn_vaani_annotated.jsonl" # UPDATE THIS PATH
    ...
    print("Starting Bengali Audio Processing and Annotation Workflow...") # Updated print
