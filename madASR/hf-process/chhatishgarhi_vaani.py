# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Devanagari script (Chhattisgarhi)
import os
import gc
import json
import re
...
class AudioSegment:
    ...
    transcription: str # From JSON metadata (Chhattisgarhi text in Devanagari)
...
class ChunkData:
    ...
    def get_formatted_text(self) -> str:
        ...
        transcription = segment.transcription.strip() # Chhattisgarhi text
...
# --- AI Annotation Functions (MODIFIED PROMPT for Chhattisgarhi) ---
...
def annotate_batch_texts(texts_to_annotate: List[str]):
    ...
    # Input texts have format: "chhattisgarhi text AGE_X GENDER_Y EMOTION_Z"
...
2.  **Entity Annotation (Chhattisgarhi Text Only):**
    *   Identify entities **only** within the main Chhattisgarhi transcription part (before the metadata).
...
3.  **Strict Entity Tagging Format:**
    ...
    *   **Example:** For "रायपुर", correct annotation is `ENTITY_CITY रायपुर END`.
...
**Examples Demonstrating Correct Formatting (Chhattisgarhi):**

*   **Input:** `"मैं कल रायपुर जाहूं। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"मैं कल ENTITY_CITY रायपुर END जाहूं। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"रमेश ह बैंक गईस। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ENTITY_PERSON_NAME रमेश END ह ENTITY_ORGANIZATION बैंक END गईस। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note: Assuming "बैंक" is Organization. Note space after each `END` unless followed by punctuation like `।`)*

*   **Input:** `"हमर बिहाव दिसंबर १५ के हे। AGE_18_30 GENDER_FEMALE EMOTION_HAPPY"`
*   **Correct Output:** `"हमर बिहाव ENTITY_DATE दिसंबर १५ END के हे। AGE_18_30 GENDER_FEMALE EMOTION_HAPPY INTENT_INFORM"`

*   **Input:** `"तंह चुप रहिबे का? AGE_45_60 GENDER_MALE EMOTION_ANNOYED"`
*   **Correct Output:** `"तंह चुप रहिबे का? AGE_45_60 GENDER_MALE EMOTION_ANNOYED INTENT_QUESTION"` *(Changed intent to QUESTION)*
...
*   `... ENTITY_CITY रायपुर ENDजाहूं ...` (Missing space after END)
*   `... गईस END AGE_...` (Unnecessary END before metadata)
*   `... ह END बैंक END ...` (Incorrectly adding END after non-entity words)
...
# --- Main Processing Function (Updated Comments for Chhattisgarhi) ---
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Chhattisgarhi audio files, extracts metadata (transcript, gender) from JSON,
    predicts age/emotion, formats text, annotates with AI (using Chhattisgarhi prompt),
    ...
    if not os.path.exists(audio_dir) or not os.path.exists(metadata_dir):
        ...
        print("Please ensure your Chhattisgarhi data follows the structure:")
        print(f"{base_dir}/\n  ├── audio/\n  └── metadata/")
        return
    ...
    # --- Process Files (Main loop - logic unchanged, handles Chhattisgarhi data) ---
    for i, (audio_path, metadata_path) in enumerate(file_pairs):
        ...
        try:
            ...
            # 4. Create Initial Record (Uses Chhattisgarhi transcript/gender from metadata)
            ...
            initial_formatted_text = chunk.get_formatted_text() # Formats with Chhattisgarhi text
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                annotated_texts = annotate_batch_texts(texts_to_annotate) # Calls Chhattisgarhi-specific prompt
...
if __name__ == "__main__":
    # --- *** Configuration for Chhattisgarhi Data (Vaani-like Structure) *** ---
    # ---> !! IMPORTANT: UPDATE THESE PATHS TO YOUR CHHATTISGARHI DATASET !! <---
    #      It should contain the 'audio' and 'metadata' subdirectories.
    BASE_AUDIO_META_DIR = "/external4/datasets/madasr/Vaani-transcription-part/Chhattisgarhi/chh" # UPDATE THIS PATH (use 'chg' or similar code)
    FINAL_OUTPUT_JSONL = "/external4/datasets/madasr/Vaani-transcription-part/Chhattisgarhi/chh/chg_vaani_annotated.jsonl" # UPDATE THIS PATH
    ...
    print("Starting Chhattisgarhi Audio Processing and Annotation Workflow...") # Updated print
