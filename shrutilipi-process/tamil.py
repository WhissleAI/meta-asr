# -*- coding: utf-8 -*- # Ensure editor recognizes UTF-8 for Tamil script comments/examples
import os
import gc
import json
import re
...
# Note: Emotion model performance might vary across languages like Tamil.
...
class AudioSegment:
    ...
    age: float
    gender: str
    transcription: str # This will hold Tamil text
    emotion: str
    chunk_filename: str
    duration: float
...
def get_transcription(text_file: str) -> str:
    try:
        # *** CRITICAL: Ensure UTF-8 for Tamil script ***
...
# --- AI Annotation Functions (MODIFIED for Tamil) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    """
    Cleans up END tags and fixes spacing around tags and common punctuation.
    Assumes Tamil primarily uses standard punctuation like '.' in modern text.
    """
    ...
    # 8. Tamil/Common punctuation spacing rules (apply last)
    # Remove space BEFORE specific punctuation (Tamil often uses '.', '?', '!')
    text = re.sub(r'\s+([?!:;,.])', r'\1', text)
    # Ensure space AFTER punctuation if followed by a word character (Tamil or Latin)
    # \w includes Unicode letters, covering Tamil script characters.
    text = re.sub(r'([?!:;,.])(\w)', r'\1 \2', text)
    # Note: If your data uses traditional Tamil punctuation extensively (e.g., ।), you might need to add it to the regex character classes.
...
# *** MODIFIED PROMPT FOR TAMIL ***
def annotate_batch_texts(texts_to_annotate: List[str]):
    """Sends a batch of texts to Gemini for annotation (Prompt adapted for Tamil)."""
    if not genai:
    ...
    # Input texts already have metadata like: "tamil text AGE_X GENDER_Y EMOTION_Z"

    # *** TAMIL PROMPT STARTS HERE ***
    prompt = f'''You are an expert linguistic annotator for **Tamil** text written in the **Tamil** script.
You will receive a list of Tamil sentences. Each sentence already includes metadata tags (AGE_*, GENDER_*, EMOTION_*) at the end.
...
2.  **ENTITY ANNOTATION (Tamil Text Only):** Identify entities ONLY within the Tamil transcription part of the sentence. Use ONLY the entity types from the provided list.
3.  **ENTITY TAG FORMAT (VERY IMPORTANT):**
    ...
    *   Immediately **AFTER** the Tamil entity text, add a single space followed by `END`.
    *   Example: `... ENTITY_CITY சென்னையில் END ...` (using Tamil script for Chennai)
    *   **DO NOT** add an `END` tag just before the `AGE_`, `GENDER_`, `EMOTION_` tags unless it belongs to a preceding entity.
4.  **INTENT TAG:** Determine the single primary intent of the Tamil transcription (e.g., INFORM, QUESTION, REQUEST, COMMAND, GREETING, etc.). Add ONE `INTENT_<INTENT_TYPE>` tag at the absolute end of the entire string, AFTER all other tags.
5.  **OUTPUT FORMAT:** Return a JSON array of strings, where each string is a fully annotated sentence adhering to all rules.
6.  **TAMIL SPECIFICS:** Handle **Tamil script**, punctuation (like the period `.`), and spacing correctly according to standard Tamil rules. Ensure proper spacing around the inserted tags.
...
**Example Input String (Tamil):**
"நான் நாளைக்கு மாரியாவை சென்னையில் பார்ப்பேன். AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL"

**CORRECT Example Output String (Tamil):**
"நான் நாளைக்கு ENTITY_PERSON_NAME மாரியா END வை ENTITY_CITY சென்னையில் END பார்ப்பேன். AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"

**INCORRECT Example Output String (Spaces in Tag):**
"நான் நாளைக்கு ENTITY_PERSON_ NAM E மாரியா END வை ENTITY_CIT Y சென்னையில் END பார்ப்பேன். AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"

**INCORRECT Example Output String (Extra END before metadata):**
"நான் நாளைக்கு ENTITY_PERSON_NAME மாரியா END வை ENTITY_CITY சென்னையில் END பார்ப்பேன். END AGE_30_45 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"


**Sentences to Annotate Now:**
{json.dumps(texts_to_annotate, ensure_ascii=False, indent=2)}
'''
    # *** TAMIL PROMPT ENDS HERE ***
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     if isinstance(sentence, str):
                          ...
                          # 2. Fix END tags and spacing (using Tamil-aware function)
...
def process_audio_and_annotate(base_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Tamil audio files, extracts metadata, gets transcriptions,
    formats text, annotates with AI (using Tamil prompt), and saves final JSONL output.
    """
    ...
    try:
        ...
        # *** CRITICAL: Ensure UTF-8 for writing Tamil output ***
    ...
    # Emotion Model (Largely Language Agnostic, but monitor performance on Tamil)
    ...
    # --- Prepare File Paths (Using base_dir for Tamil data) ---
    ...
    if not os.path.exists(audio_dir) or not os.path.exists(text_dir):
        ...
        print("Please ensure your Tamil data follows the structure:")
        print(f"{base_dir}/")
        print(f"  ├── audio/  (contains .flac/.wav/.mp3 files)")
        print(f"  └── text/   (contains .txt files with matching basenames, UTF-8 encoded)")
        return

    print(f"Processing Tamil files from:\n  Audio: {audio_dir}\n  Text:  {text_dir}") # <-- Changed to Tamil
    ...
    for i, (audio_path, text_path) in enumerate(file_pairs):
        ...
        try:
            # 1. Get Tamil Transcription (Ensures UTF-8)
            ...
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                # Call Gemini for annotation (with retries and Tamil prompt)
                ...
                if len(annotated_texts) != current_batch_size:
                    ...
                    try:
                        ...
                        # *** CRITICAL: Ensure UTF-8 for appending Tamil output ***
                        with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
                            for record_data, annotated_text in zip(processed_records_buffer, annotated_texts):
                                ...
                                # *** CRITICAL: ensure_ascii=False for Tamil ***
...
if __name__ == "__main__":
    # --- *** Configuration for Tamil Data *** ---
    # 1. SET THE BASE DIRECTORY CONTAINING YOUR TAMIL 'audio' and 'text' subfolders
    BASE_AUDIO_TEXT_DIR = "/external4/datasets/Shrutilipi/tamil/tm/"  # <--- CHANGE THIS TO YOUR TAMIL DATA PATH

    # 2. SET THE DESIRED OUTPUT FILENAME FOR THE ANNOTATED TAMIL DATA
    FINAL_OUTPUT_JSONL = "/external4/datasets/Shrutilipi/tamil/tm/ta_annotated.jsonl" # <--- CHANGE THIS (e.g., ta for Tamil)
    ...
    print("Starting Tamil Audio Processing and Annotation Workflow...") # Updated print
