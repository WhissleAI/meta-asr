...
# Reference JSONL configuration for skipping already processed data
REFERENCE_JSONL = os.getenv('REFERENCE_JSONL')
if not REFERENCE_JSONL:
    _default_ref = "/external3/databases/hf-omini-data/merge_data.jsonl"
    if os.path.exists(_default_ref):
        REFERENCE_JSONL = _default_ref
_REFERENCE_ABS: set[str] = set()
_REFERENCE_BASENAMES: set[str] = set()

def _load_reference_sets(force: bool = False) -> Tuple[set, set]:
    """Load and cache reference audio paths from JSONL for skip-by-path logic.
    Recognized keys: audio_filepath, question_audio_path, audio_path, path.
    Returns (abs_paths_set, basenames_set).
    """
    global _REFERENCE_ABS, _REFERENCE_BASENAMES
    if _REFERENCE_ABS and _REFERENCE_BASENAMES and not force:
        return _REFERENCE_ABS, _REFERENCE_BASENAMES
    _REFERENCE_ABS = set()
    _REFERENCE_BASENAMES = set()
    if REFERENCE_JSONL and os.path.exists(REFERENCE_JSONL):
        loaded = 0
        try:
            with open(REFERENCE_JSONL, 'r', encoding='utf-8') as rf:
                for line in rf:
                    try:
                        obj = json.loads(line)
                    except Exception:
                        continue
                    ap = (
                        obj.get('audio_filepath')
                        or obj.get('question_audio_path')
                    ...
                    try:
                        ...
                        _REFERENCE_ABS.add(ap_abs)
                        _REFERENCE_BASENAMES.add(os.path.basename(ap_abs))
                        loaded += 1
                    except Exception:
                        continue
            print(f"[reference-jsonl] Loaded {loaded} paths from {REFERENCE_JSONL}. unique_abs={len(_REFERENCE_ABS)}")
        except Exception as e:
            print(f"[reference-jsonl] Failed to read {REFERENCE_JSONL}: {e}")
    elif REFERENCE_JSONL:
        print(f"[reference-jsonl] Reference JSONL not found: {REFERENCE_JSONL}")
    return _REFERENCE_ABS, _REFERENCE_BASENAMES
...
class AudioSegment:
    ...
    car_structured: Dict[str, Any] = field(default_factory=dict)
...
def gemini_transcribe_segment(audio_path: str) -> str:
    """Transcribe an audio segment using Gemini for automotive content.
    Returns empty string on failure. Adds optional debug prints if GEMINI_DEBUG=1.
    Guidance emphasizes preserving technical terms, codes, and units relevant to cars.
    """
    if not _init_gemini():
        return ""
    try:
        ...
        with open(audio_path, 'rb') as f:
            audio_bytes = f.read()
        # Prompt tailored for automotive/vehicle repair and diagnostics content
        prompt = (
            "You are transcribing automotive audio (vehicle repair, diagnostics, specs). "
            "Output a faithful, verbatim transcript in ENGLISH ONLY. "
            "Preserve technical terms, part names, OBD-II codes (e.g., P0300), numeric values and UNITS (e.g., 12 V, 10 Nm, 32 psi), "
            "and measurements exactly as spoken. Do NOT add brackets or labels, and do NOT include speaker names or timestamps. "
            "Keep original phrasing; do not paraphrase or summarize. If another language appears, provide a natural English transliteration/translation inline without brackets. "
...
def gemini_entities_intents(text: str) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    try:
        ...
        prompt = (
            "You are an NLP annotator for AUTOMOTIVE content (cars, repair, diagnostics).\n"
            "Given the transcript text, extract: \n"
            "  - Entities with types from: VEHICLE, PART, SYSTEM, ISSUE, OBD_CODE, MEASUREMENT, TOOL, ACTION, LOCATION, FLUID, WARNING, BRAND, MODEL, YEAR, SOFTWARE.\n"
            "    • VEHICLE: general vehicle mentions or specific references (e.g., 'car', 'SUV', '2015 Civic').\n"
            "    • BRAND/MODEL/YEAR: vehicle make, model, and year if present.\n"
            "    • SYSTEM: vehicle subsystems (engine, transmission, cooling, electrical, HVAC, brakes, suspension, fuel, exhaust, drivetrain, ADAS).\n"
            "    • PART: specific components (e.g., 'oxygen sensor', 'alternator', 'coil pack', 'radiator cap').\n"
            "    • ISSUE: problems/symptoms/diagnoses (e.g., 'misfire', 'overheating', 'no crank', 'rattling noise').\n"
            "    • OBD_CODE: diagnostic trouble codes (e.g., 'P0420', 'C1234').\n"
            "    • MEASUREMENT: values with units (e.g., '12 V', '32 psi', '90 °C', '10 Nm').\n"
            "    • TOOL: tools/equipment (e.g., 'OBD scanner', 'multimeter', 'torque wrench').\n"
            "    • ACTION: repair or diagnostic steps (e.g., 'replace spark plugs', 'bleed brakes', 'reset ECU').\n"
            "    • LOCATION: vehicle-specific locations (e.g., 'bank 1 sensor 2', 'cylinder 3', 'front left').\n"
            "    • FLUID: automotive fluids and specs (e.g., '5W-30', 'DOT 4', 'coolant').\n"
            "    • WARNING: explicit safety cautions.\n"
            "    • SOFTWARE: ECU/firmware updates, calibrations.\n"
            "  - Intents from: DIAGNOSIS, INSTRUCTION, SAFETY_WARNING, QUESTION, TROUBLESHOOTING, SPECIFICATION, RECOMMENDATION, NOTE.\n"
...
def gemini_car_structured(text: str) -> Dict[str, Any]:
    """Extract a structured automotive summary from text using Gemini.
    Returns a dictionary with keys like: vehicle{make, model, year, trim}, systems[], parts[], issues[],
    obd_codes[], actions[], tools[], measurements[{name,value,unit}], locations[], fluids[], safety_warnings[],
    recommendations[], software[]. Empty dict on failure or if Gemini is not configured.
    """
    if not text or not text.strip():
        return {}
    if not _init_gemini():
    ...
    try:
        ...
        truncated = text[:6000]
        prompt = (
            "Return ONLY valid JSON capturing a structured automotive summary from the transcript text.\n"
            "Schema: {\n"
            "  \"vehicle\": {\"brand\": null|str, \"model\": null|str, \"year\": null|int, \"trim\": null|str},\n"
            "  \"systems\": [str],\n"
            "  \"parts\": [str],\n"
            "  \"issues\": [str],\n"
            "  \"obd_codes\": [str],\n"
            "  \"actions\": [str],\n"
            "  \"tools\": [str],\n"
            "  \"measurements\": [{\"name\": str, \"value\": number|string, \"unit\": str}],\n"
            "  \"locations\": [str],\n"
            "  \"fluids\": [str],\n"
            "  \"safety_warnings\": [str],\n"
            "  \"recommendations\": [str],\n"
            "  \"software\": [str]\n"
            "}.\n"
            "Use null for unknown scalar fields; use empty arrays when nothing is present.\n"
            "Text: \"" + truncated.replace('"','\\"') + "\""
        )
        resp = model.generate_content(prompt)
        raw = getattr(resp, 'text', '') or '{}'
        ...
        try:
            ...
            if os.getenv('GEMINI_DEBUG','0') == '1':
                print(f"[gemini] structured parse failed, raw response: {raw}")
            return {}
    except Exception as e:
        if os.getenv('GEMINI_DEBUG','0') == '1':
...
def process_large_audio(
    ...
    # Load reference sets (if any) to allow skipping already-processed chunks by path
    ref_abs, ref_basenames = _load_reference_sets()
    skipped_chunks_due_to_reference = 0
    ...
    try:
        ...
        for chunk_start in range(0, len(signal), stride):
            ...
            # Skip this chunk if already present in reference JSONL (match by abs path or basename)
            try:
                cp_abs = os.path.realpath(os.path.abspath(chunk_path))
            except Exception:
                cp_abs = os.path.abspath(chunk_path)
            cp_base = os.path.basename(cp_abs)
            if (cp_abs in ref_abs) or (cp_base in ref_basenames):
                skipped_chunks_due_to_reference += 1
                # Still increment chunk_counter and move on without writing anything
                chunk_counter += 1
                continue
            ...
            try:
                ...
                for speaker_idx, (start_time, end_time, speaker) in enumerate(speaker_changes):
                    ...
                    try:
                        ...
                        # Structured automotive extraction
                        car_structured = gemini_car_structured(base_text_for_ner)
        ...
        if all_data:
            ...
            if skipped_chunks_due_to_reference:
                print(f"[reference-jsonl] Skipped {skipped_chunks_due_to_reference} chunks already present in {REFERENCE_JSONL}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        download_dir = "/external4/datasets/strem2action-audio/creative_design"
