...
# --- AI Annotation Functions (MODIFIED for Bhojpuri) ---
...
def fix_end_tags_and_spacing(text: str) -> str:
    ...
    spacing around tags, internal spaces in tag types, and Devanagari punctuation.
    ...
    # 8. Devanagari/Common punctuation spacing rules
    #    Remove space BEFORE specific punctuation (Includes Devanagari Danda '।')
    text = re.sub(r'\s+([?!:;,.।])', r'\1', text) # Re-added Devanagari Danda '।'
    #    Ensure space AFTER punctuation if followed by a word character (\w handles Devanagari)
    text = re.sub(r'([?!:;,.।])(\w)', r'\1 \2', text) # Re-added Devanagari Danda '।'
...
*   **Input:** `"हमर नाम रमेश हऽ। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"हमर नाम ENTITY_PERSON_NAME रमेश END हऽ। AGE_30_45 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"ऊ पटना में रहेलें। AGE_45_60 GENDER_FEMALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"ऊ ENTITY_CITY पटना END में रहेलें। AGE_45_60 GENDER_FEMALE EMOTION_NEUTRAL INTENT_INFORM"`
    *(Note the space after each `END` tag)*

*   **Input:** `"बैठक 10 मई के होई। AGE_18_30 GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"बैठक ENTITY_DATE 10 मई END के होई। AGE_18_30 GENDER_MALE EMOTION_NEUTRAL INTENT_INFORM"`

*   **Input:** `"कृपया शांत रहीं? AGE_60PLUS GENDER_MALE EMOTION_NEUTRAL"`
*   **Correct Output:** `"कृपया शांत रहीं? AGE_60PLUS GENDER_MALE EMOTION_NEUTRAL INTENT_REQUEST"`
...
*   `... रहेलें END AGE_...` (Unnecessary END before metadata)
        ...
        try:
            ...
            if isinstance(annotated_sentences_raw, list) and len(annotated_sentences_raw) == len(texts_to_annotate):
                ...
                for idx, sentence in enumerate(annotated_sentences_raw):
                     if isinstance(sentence, str): processed_sentences.append(fix_end_tags_and_spacing(sentence)) # Apply Bhojpuri/Devanagari fixing rules
                     else:
...
# --- Main Processing Function (Handles Bhojpuri data structure) ---
def process_audio_and_annotate(input_dir: str, output_jsonl_path: str, batch_size: int = 10) -> None:
    """
    Processes Bhojpuri audio files from a single directory containing .wav files and data.json,
    extracts metadata, gets transcriptions from data.json, formats text,
    annotates with AI (using Bhojpuri prompt), and saves final JSONL output.
    """
    ...
    try:
        print(f"Loading Bhojpuri transcriptions from {json_path}...")
    ...
    for i, wav_filename in enumerate(wav_files):
        ...
        try:
            ...
            # 6. Annotate and Save in Batches (Uses Bhojpuri annotation function)
            if len(processed_records_buffer) >= batch_size or (i + 1) == total_files:
                ...
                annotated_texts = annotate_batch_texts(texts_to_annotate) # Uses Bhojpuri-specific func
    ...
    print("\n" + "="*30); print("Processing Finished (Bhojpuri).")
...
if __name__ == "__main__":
    ...
    # >>> IMPORTANT: CHANGE THIS PATH TO YOUR BHOJPURI DATA DIRECTORY <<<
    # This directory should contain your .wav files AND the data.json file
    INPUT_DATA_DIR = "/external4/datasets/madasr/bhojpuri-ekstep"  # Example: /data/bhojpuri_corpus/set1
    # >>> IMPORTANT: SET YOUR DESIRED OUTPUT FILE PATH <<<
    FINAL_OUTPUT_JSONL = "/external4/datasets/madasr/ekstep-jsonl/annotated_bhojpuri_output.jsonl" # Example: /data/bhojpuri_corpus/set1/annotated_output.jsonl
    ...
    # Start the main processing function for Bhojpuri
    ...
    print("Bhojpuri Workflow complete.")