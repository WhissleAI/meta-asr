...
def remove_existing_tags(text):
    ...
    Handles Serbian accented characters properly.
    """
    preserved_tags = {
...
def fix_end_tags(text):
    ...
    Handles Serbian text with proper spacing rules.
    """
    if not isinstance(text, str):
        return text

    # Handle Serbian-specific punctuation spacing
...
def annotate_sentences(sentences):
    prompt = f'''
You are given a list of Serbian sentences that may contain existing tags like GENDER_FEMALE, EMOTION_NEU, AGE_45_60, and SPEAKER_CHANGE.

Your task is to:
1. Remove any legacy entity tags such as NER_PERSON, NER_NORP, etc.
2. Preserve these existing tags exactly as they appear.
3. **ONLY annotate entities from the following list in Serbian text.** Do NOT invent new entity types.
4. Insert entity tags in the format ENTITY_<TYPE> before each entity and append " END" after it.
5. Focus on identifying Serbian entities that match the provided list.
6. Classify the **intent** of each Serbian sentence and add an **INTENT_<INTENT_TYPE>** tag at the end.
7. Return the output as a JSON array of annotated sentences.

IMPORTANT:
- Maintain proper Serbian spacing rules (space before :;?! and no space before .,)
- Handle Serbian accented characters and special characters properly
- Follow Serbian capitalization rules for proper nouns and entities
...
Input: "Видео сам Марију у Београду јуче. AGE_30_40 GENDER_FEMALE EMOTION_JOY SPEAKER_CHANGE"
Output: "Видео сам ENTITY_PERSON_NAME Марију END у ENTITY_CITY Београду END јуче. AGE_30_40 GENDER_FEMALE EMOTION_JOY SPEAKER_CHANGE INTENT_INFORM"
...
def process_jsonl_file(input_path, output_path, batch_size=10):
    """
    Process Serbian JSONL file with proper character encoding and Serbian text handling.
    """
...
if __name__ == "__main__":
    input_jsonl_path = "/hydra2-prev/home/compute/workspace_himanshu/Processed_Data/gemini_metadata/salvic/serbanian_uni.jsonl"
    output_jsonl_path = "/hydra2-prev/home/compute/workspace_himanshu/Processed_Data/gemini_metadata/salvic/processed/serbian.jsonl"
